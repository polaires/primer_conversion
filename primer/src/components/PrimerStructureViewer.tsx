...
import { foldSequence, dg } from '../lib/fold.js';
import FornaViewer from './FornaViewer';
...
interface PrimerStructureViewerProps {
  ...
  isSDMMode?: boolean;
  onForwardShift?: ((direction: number) => void) | null;
  onReverseShift?: ((direction: number) => void) | null;
...
function calculatePositionwiseDG(sequence: string, windowSize: number = 10): number[] {
  ...
  for (let i = 0; i < sequence.length; i++) {
    ...
    let energy = subseq.length >= 6 ? (dg as any)(subseq, 37) : 0;
    if (!isFinite(energy)) {
      energy = 0;
    }
...
function calculatePairingProbability(sequence: string, index: number, basePairs: Array<[number, number]>): number {
...
const ForceDirectedStructure: FC<ForceDirectedStructureProps> = ({
  sequence,
  basePairs = [],
  width = 300,
  height = 250,
        ...
        if (isPaired) {
          ...
          if (i >= loopStart && i <= loopEnd) {
            ...
            for (const [a, b] of sortedPairs) {
              if (i > a && i < b) {
                const pairIdx = sortedPairs.findIndex(([pa]) => pa === a);
                const stemLevel = stemPairCount - 1 - pairIdx;
  ...
  const viewBox = useMemo(() => {
    ...
    const maxY = Math.max(...ys) + padding + 30;
...
const CrossDimerZipper: FC<CrossDimerZipperProps> = ({
  forwardSeq,
  reverseSeq,
  width: _width = 600,
  height: _height = 200,
      ...
      <div style={{ background: theme.bgTertiary, borderRadius: '6px', padding: '16px', fontFamily: 'monospace', fontSize: '12px', overflowX: 'auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', marginBottom: '4px', whiteSpace: 'nowrap' }}>
          ...
          {forwardSeq.split('').map((base, i) => {
            ...
            return (
              <span key={i} style={{ color, background: bg, padding: '0 1px', fontWeight: isPaired ? 'bold' : 'normal', display: 'inline-block', width: '10px', textAlign: 'center' }}>
                {base}
              </span>
            );
          })}
...
const ThermodynamicHeatmap: FC<ThermodynamicHeatmapProps> = ({
  sequence,
  templateRegion: _templateRegion = null,
  width: _width = 600,
  label = 'Primer'
}) => {
...
const PrimerStructureViewer: FC<PrimerStructureViewerProps> = ({
  forwardSeq,
  reverseSeq,
  ...
  const fwdFold = useMemo((): FoldResult => {
    if (!forwardSeq || forwardSeq.length < 6) return { e: 0, ij: [] };
    return (foldSequence as any)(forwardSeq) as FoldResult;
  }, [forwardSeq]);

  const revFold = useMemo((): FoldResult => {
    if (!reverseSeq || reverseSeq.length < 6) return { e: 0, ij: [] };
    return (foldSequence as any)(reverseSeq) as FoldResult;
  }, [reverseSeq]);

  const calcDimensions = (seq: string | undefined, pairs: Array<[number, number]> | undefined): Dimensions => {
...
export default PrimerStructureViewer;

export { ForceDirectedStructure, CrossDimerZipper, ThermodynamicHeatmap };
