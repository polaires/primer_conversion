...
// the energies are the same for each loop stack in the reverse complementary direction
Object.keys(DNA_NN).forEach((k) => {
...
Object.keys(DNA_INTERNAL_MM).forEach((k) => {
  const rev = reverseString(k);
  if (!(rev in DNA_INTERNAL_MM)) {
    DNA_INTERNAL_MM[rev] = DNA_INTERNAL_MM[k];
  }
});

Object.keys(DNA_TERMINAL_MM).forEach((k) => {
  const rev = reverseString(k);
  if (!(rev in DNA_TERMINAL_MM)) {
    DNA_TERMINAL_MM[rev] = DNA_TERMINAL_MM[k];
  }
});

Object.keys(DNA_DE).forEach((k) => {
  const rev = reverseString(k);
  if (!(rev in DNA_DE)) {
    DNA_DE[rev] = DNA_DE[k];
  }
