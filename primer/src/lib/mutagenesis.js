...
import { DNA_INTERNAL_MM, DNA_TERMINAL_MM, DNA_DE, DNA_NN } from './dna.js';
 ...
 * @param {string} structureType - 'hairpin', 'selfDimer', or 'heterodimer'
 * @param {boolean} involves3Prime - Whether 3' end is involved
 * @returns {Object} Threshold object with ideal, warning, critical values
 */
export function getThreshold(structureType, involves3Prime = false) {
  const thresholds = DIMER_THRESHOLDS[structureType];
  if (!thresholds) {
 ...
 * @param {number} deltaG - The ΔG value in kcal/mol
 * @param {string} structureType - 'hairpin', 'selfDimer', or 'heterodimer'
 * @param {boolean} involves3Prime - Whether 3' end is involved
 * @returns {string} 'pass', 'warning', or 'fail'
 */
export function classifyDimerSeverity(deltaG, structureType, involves3Prime = false) {
...
export const MUTAGENESIS_DEFAULTS = {
 ...
 */
function isWatsonCrickPair(base1, base2) {
 ...
 */
function getMismatchKey(primerDinuc, templateStrandDinuc) {
 ...
 *
 * @param {string} primerSeq - Primer sequence (5'→3')
 * @param {string} templateSeq - Coding/sense strand sequence (5'→3') - will be converted
 *                               to template strand internally for NN calculation
 * @param {Object} options - Calculation options
 * @returns {Object} Tm calculation result with details
 */
export function calculateMismatchedTm(primerSeq, templateSeq, options = {}) {
  const {
    primerConc = 500, // nM
 ...
 */
export function checkMutantSecondaryStructure(mutantPrimer, originalPrimer = null, options = {}) {
 ...
 */
function check3primeInvolvement(seq, structures) {
  const n = seq.length;
 ...
 * @param {number} options.threshold - ΔG threshold for warning (default from DIMER_THRESHOLDS)
 * @param {number} options.criticalThreshold - ΔG threshold for critical warning
 * @param {string} options.designType - Design type: 'overlapping', 'back-to-back', or undefined
 */
export function checkHeterodimer(primer1, primer2, options = {}) {
 ...
 */
function estimateHairpinDG(seq) {
  let minDG = 0;
 ...
 */
function estimateSelfDimerDG(seq) {
 ...
 *
 * @param {string} originalCodon - Current codon
 * @param {string} targetAA - Target amino acid (one-letter code)
 * @param {Object} options - Selection options
 * @returns {Object} Selected codon with details
 */
export function selectOptimalCodon(originalCodon, targetAA, options = {}) {
 ...
 */
export function checkPrimerSpecificity(primer, fullTemplate, maxMismatches = 3) {
 ...
 * @param {number} mutPosition - Mutation position
 * @param {string} mutatedSeq - Mutated template sequence
 * @param {Object} options - Design options
 */
function designBackToBackPrimers(template, mutPosition, mutatedSeq, mutLength, options, deletionLength = 0) {
  ...
  let max5primeFlank;
  let min5primeFlank;  // Minimum 5' context for binding stability
  if (isDeletion) {
 ...
 */
function designOverlappingPrimers(template, mutPosition, mutatedSeq, mutLength, options) {
  const opts = { ...MUTAGENESIS_DEFAULTS, ...options };
  const mutSeq = mutatedSeq.toUpperCase();
 ...
 */
function designMutagenisPrimerPair(originalSeq, mutatedSeq, mutPosition, mutLength, opts, isInsertion = false, deletionLength = 0) {
  let allCandidates = [];
  ...
  // =================================================================
  // For substitutions and insertions, we can shift where the back-to-back
  // junction occurs by ±1-2 bases. This helps avoid problematic sequences
  // like homopolymer runs (GGGG) or palindromes at the junction.
  ...
  const collectedDiagnostics = {
    ...
    noFwdCandidatesInTmWindow: 0,
    noRevCandidatesInTmWindow: 0,
    fwdTmTooLow: { count: 0, lowestTm: Infinity, sequence: null, gc: null },
    fwdTmTooHigh: { count: 0, highestTm: -Infinity, sequence: null, gc: null },
    revTmTooLow: { count: 0, lowestTm: Infinity, sequence: null, gc: null },
    revTmTooHigh: { count: 0, highestTm: -Infinity, sequence: null, gc: null },
  ...
  for (const offset of splitPointOffsets) {
    ...
    if (opts.strategy === 'back-to-back') {
      ...
      if (diagnostics) {
        collectedDiagnostics.totalPositionsExplored += diagnostics.positionsExplored;
        collectedDiagnostics.totalPairsEvaluated += diagnostics.pairsEvaluated;
        collectedDiagnostics.fwdRegionTooShort += diagnostics.fwdRegionTooShort;
        collectedDiagnostics.revRegionTooShort += diagnostics.revRegionTooShort;
        collectedDiagnostics.noFwdCandidatesInTmWindow += diagnostics.noFwdCandidatesInTmWindow;
        collectedDiagnostics.noRevCandidatesInTmWindow += diagnostics.noRevCandidatesInTmWindow;
        ...
        collectedDiagnostics.fwdTmTooLow.count += diagnostics.fwdTmTooLow.count;
        ...
        collectedDiagnostics.fwdTmTooHigh.count += diagnostics.fwdTmTooHigh.count;
        ...
        collectedDiagnostics.revTmTooLow.count += diagnostics.revTmTooLow.count;
        ...
        collectedDiagnostics.revTmTooHigh.count += diagnostics.revTmTooHigh.count;
      }
    } else {
  ...
  if (allCandidates.length === 0) {
    ...
    if (d.noFwdCandidatesInTmWindow > 0 || d.noRevCandidatesInTmWindow > 0) {
      ...
      if (d.revTmTooLow.count > 0 && d.revTmTooLow.lowestTm !== Infinity) {
        const gc = d.revTmTooLow.gc ? ` (GC: ${(d.revTmTooLow.gc * 100).toFixed(0)}%)` : '';
        errorParts.push(`  - Reverse primers too cold: best achieved ${d.revTmTooLow.lowestTm}°C${gc}`);
        suggestions.push('lower minimum Tm');
      }

      if (d.revTmTooHigh.count > 0 && d.revTmTooHigh.highestTm !== -Infinity) {
        const gc = d.revTmTooHigh.gc ? ` (GC: ${(d.revTmTooHigh.gc * 100).toFixed(0)}%)` : '';
        errorParts.push(`  - Reverse primers too hot: lowest achieved ${d.revTmTooHigh.highestTm}°C${gc}`);
        suggestions.push('increase maximum Tm');
      }
    }
  ...
  allCandidates.sort((a, b) => a.penalty - b.penalty);

  // Take top N candidates for detailed analysis
  // Exhaustive search evaluates more candidates for optimal result (but slower)
  const TOP_N = opts.exhaustiveSearch ? 100 : 10;
  const topCandidates = allCandidates.slice(0, TOP_N);

  if (opts.exhaustiveSearch) {
    console.log(`[mutagenesis] Exhaustive search: evaluating ${topCandidates.length} candidates (of ${allCandidates.length} total)`);
  }

  // Run expensive calculations on top candidates only (dg, mismatch Tm, off-target)
  for (const candidate of topCandidates) {
    // Calculate dg (secondary structure - expensive fold)
    ...
    // (Not applicable for deletions since primer spans the junction)
    if (candidate.forward._templateRegion) {
      try {
        const mismatchResult = calculateMismatchedTm(
          candidate.forward.sequence,
          candidate.forward._templateRegion
        );
        ...
        candidate.forward.mismatchFraction = mismatchResult.mismatchFraction;
        if (mismatchResult.hasCritical3primeMismatch) {
          candidate.penalty += 20;
        }
        // Heavy penalty if primer won't bind (too many mismatches)
        if (mismatchResult.willNotBind) {
          candidate.penalty += 100;
        }
      } catch (e) {
        // Mismatch calculation failed (e.g., length mismatch) - skip it
        console.warn('Mismatch Tm calculation failed:', e.message);
      }
      delete candidate.forward._templateRegion; // Clean up temp data
    }
    // Clean up deletion flag
    if (candidate.forward._isDeletion !== undefined) {
      candidate.forward.isDeletion = candidate.forward._isDeletion;
      delete candidate.forward._isDeletion;
    }
    ...
    candidate.forward.offTargets = fwdSpec.offTargetCount;
    candidate.reverse.offTargets = revSpec.offTargetCount;
    candidate.penalty += fwdSpec.offTargetCount * 5;
    candidate.penalty += revSpec.offTargetCount * 5;

    // =================================================================
    // NEW HEURISTICS: 3' Terminal Stability, G-Quadruplex, Terminal Base
    // Run only on top candidates (after expensive calculations)
    // =================================================================

    // A. 3' Terminal Delta G (last 5 bases stability)
    ...
    candidate.forward.terminalDG = fwdTermDG;
    candidate.reverse.terminalDG = revTermDG;

    // Ideal range: -6 to -11 kcal/mol
    // > -6: too loose (risk of poor initiation)
    // < -12: too sticky (risk of mispriming during ramp-up)
    if (fwdTermDG.dG > -6.0) {
      candidate.penalty += 5;
    }
    ...
    candidate.forward.terminalBase = fwdTermBase;
    candidate.reverse.terminalBase = revTermBase;
    candidate.penalty += fwdTermBase.penalty + revTermBase.penalty;

    // C. G-Quadruplex Detection (hard check)
    // G4 structures cause polymerase arrest - Q5 struggles with these
    const fwdG4 = checkGQuadruplexRisk(candidate.forward.sequence);
    const revG4 = checkGQuadruplexRisk(candidate.reverse.sequence);
    candidate.forward.gQuadruplex = fwdG4;
    candidate.reverse.gQuadruplex = revG4;

    if (fwdG4.hasG4Motif) {
      candidate.penalty += 1000; // Effectively disqualifies
    } else if (fwdG4.hasGGGG) {
      candidate.penalty += 50; // Warning-level penalty
    }
    ...
    // =================================================================
    // COMPOSITE SCORE: Calculate calibrated 0-100 score for tier selection
    // This enables consistent quality assessment across all primer types
    // =================================================================
    ...
    // Use equilibrium-based calculations at estimated annealing temp
    const estAnnealTemp = Math.min(fwdTm, revTm) - 2; // Rough estimate
    ...
    // Get terminal DG (already calculated above)
    const fwdTermDGVal = candidate.forward.terminalDG?.dG ?? -8;
    const revTermDGVal = candidate.reverse.terminalDG?.dG ?? -8;

    // Helper to validate and sanitize numeric values
    const sanitize = (val, fallback = 0.5) => Number.isFinite(val) ? val : fallback;
    ...
    // Calculate piecewise scores with NaN/Infinity validation
    const candidateScores = {
      ...
      offTarget: Math.min(
        sanitize(scoreOffTarget(candidate.forward.offTargets || 0), 1),
        sanitize(scoreOffTarget(candidate.reverse.offTargets || 0), 1)
      ),
      // G-Quadruplex scoring - critical for Q5 polymerase (0.16 total weight)
  ...
  // =================================================================
  // TIERED SELECTION: Pick from best quality tier first
  // =================================================================
  // This prevents a candidate with perfect GC from "outvoting" Tm matching.
  // We first filter by hard thresholds, then sort within tiers.
  ...
  // Check secondary structure of mutant primer
  ...
  // Use tier-based quality if available, otherwise fall back to penalty-based
  ...
  // Collect all warnings from the selected design
  ...
  // Add warning for short primers (< 15bp) - may have reduced specificity
  if (best.reverse.length < 15) {
    warnings.push({
      type: 'short_primer',
      severity: 'info',
      message: `Reverse primer is ${best.reverse.length}bp (shorter than typical 15bp) to achieve better Tm matching. High GC content (${best.reverse.gcPercent}) maintains adequate Tm. Consider BLAST check for specificity.`,
    });
  }
  if (best.forward.length < 15) {
    warnings.push({
      type: 'short_primer',
      severity: 'info',
      message: `Forward primer is ${best.forward.length}bp (shorter than typical 15bp). Consider BLAST check for specificity.`,
    });
  }

  // =================================================================
  // CALIBRATED SCORING (Döring dataset, F1=81.9%, AUC=0.848)
  // =================================================================
  // Helper function to calculate scoring for a design
  const calculateDesignScoring = (design) => {
    ...
    // Calculate equilibrium-based hairpin/homodimer/heterodimer ΔG at annealing temp
    ...
    // Calculate 3' terminal stability (use existing or calculate)
    const fwdTermDG = design.forward.terminalDG?.dG ?? calculate3primeTerminalDG(fwdSeq).dG;
    const revTermDG = design.reverse.terminalDG?.dG ?? calculate3primeTerminalDG(revSeq).dG;

    // Calculate individual piecewise scores (0-1 each, will be weighted)
    // Note: calculateHairpinDG, calculateHomodimerDG, calculateHeterodimerDG return numbers directly
    // But calculate3primeTerminalDG returns an object with .dG property
    const piecewiseScores = {
      ...
      hairpinFwd: scoreHairpin(fwdHairpinDG),       // fwdHairpinDG is already a number
      hairpinRev: scoreHairpin(revHairpinDG),       // revHairpinDG is already a number
      selfDimerFwd: scoreHomodimer(fwdHomodimerDG), // fwdHomodimerDG is already a number
      selfDimerRev: scoreHomodimer(revHomodimerDG), // revHomodimerDG is already a number
      heterodimer: scoreHeterodimer(heterodimerDG), // heterodimerDG is already a number
      ...
      offTargetFwd: scoreOffTarget(design.forward.offTargets || 0),
      offTargetRev: scoreOffTarget(design.reverse.offTargets || 0),
    };

    // Pass scores with keys matching DEFAULT_WEIGHTS for proper weighting
    const compositeInput = {
      tmFwd: piecewiseScores.tmFwd,
      tmRev: piecewiseScores.tmRev,
      gcFwd: piecewiseScores.gcFwd,
      gcRev: piecewiseScores.gcRev,
      terminal3DG: Math.min(piecewiseScores.terminal3DGFwd, piecewiseScores.terminal3DGRev), // Worst case
      ...
      offTarget: Math.min(piecewiseScores.offTargetFwd, piecewiseScores.offTargetRev), // Worst case
    };

    // Calculate composite score (0-100, higher = better)
    const scoreResult = calculateCompositeScore(compositeInput);
    const compositeScore = scoreResult.score; // Extract score from object
  ...
  // =================================================================
  // SAME-STRATEGY ALTERNATIVES: Get alternatives from the same strategy
  // (e.g., other back-to-back options when using back-to-back strategy)
  // =================================================================
  const sameStrategyAlternates = topCandidates
    .filter(alt =>
      // Filter out the selected best design
      alt.forward.sequence !== best.forward.sequence ||
      alt.reverse.sequence !== best.reverse.sequence
    )
    // Remove duplicates (keep first occurrence by sequence)
    .filter((alt, idx, arr) =>
      arr.findIndex(a =>
    ...
    .slice(0, 5) // Take top 5 same-strategy alternatives
    .map(alt => {
  ...
  // =================================================================
  // CROSS-STRATEGY ALTERNATIVES: Get alternatives from the other strategy
  // (e.g., QuikChange options when using back-to-back strategy)
  // =================================================================
  const scoredCrossStrategyAlternates = crossStrategyAlternates
    .filter(alt =>
      // Filter out any design with identical sequences to the current best
      alt.forward.sequence !== best.forward.sequence ||
      alt.reverse.sequence !== best.reverse.sequence
    )
    // Remove duplicates within alternates (keep first occurrence by sequence)
    .filter((alt, idx, arr) =>
      arr.findIndex(a =>
        a.forward.sequence === alt.forward.sequence &&
        a.reverse.sequence === alt.reverse.sequence
  ...
  return {
    ...
    quality: bestScoring.qualityTier, // Use calibrated quality tier (string: excellent/good/acceptable/marginal/poor)
    ...
    structureCheck,
    splitPointOffset: best.splitPointOffset || 0,
    // Same-strategy alternatives (e.g., other back-to-back designs)
    alternateDesigns: sameStrategyAlternates,
    // Cross-strategy alternatives (e.g., QuikChange when using back-to-back)
    crossStrategyAlternates: scoredCrossStrategyAlternates,
    protocol: generateProtocol(best.forward.tm, best.reverse.tm, annealingResult.annealingTemp, best.design),
    warnings: warnings.length > 0 ? warnings : undefined,
  };
}

/**
 * Design primers for point mutation
 */
export function designSubstitutionPrimers(template, position, newBase, options = {}) {
  const opts = { ...MUTAGENESIS_DEFAULTS, ...options };
  const seq = template.toUpperCase();
...
/**
 * Design primers for codon change
 */
export function designCodonChangePrimers(template, codonPosition, newAA, options = {}) {
  const opts = { ...MUTAGENESIS_DEFAULTS, ...options };
  const seq = template.toUpperCase();
  ...
  return {
    ...
    alternativeCodons: codonSelection.allCandidates.filter(c => c.codon !== bestCodon),
    ...primers,
  };
}

/**
 * Design primers for insertion
 */
export function designInsertionPrimers(template, position, insertSequence, options = {}) {
  const opts = { ...MUTAGENESIS_DEFAULTS, ...options };
  const seq = template.toUpperCase();
...
/**
 * Design primers for deletion
 */
export function designDeletionPrimers(template, startPosition, length, options = {}) {
  const opts = { ...MUTAGENESIS_DEFAULTS, ...options };
  const seq = template.toUpperCase();
 ...
 */
export function designRegionSubstitutionPrimers(template, startPosition, deleteLength, replacementSeq, options = {}) {
  const opts = { ...MUTAGENESIS_DEFAULTS, ...options };
  const seq = template.toUpperCase();
...
function generateProtocol(fwdTm, revTm, annealingTemp, design) {
...
export function parseMutationNotation(notation) {
  ...
  if (aaMatch) {
    return {
      type: MUTATION_TYPES.CODON_CHANGE,
      oldAA: aaMatch[1],
      position: parseInt(aaMatch[2]),
      newAA: aaMatch[3],
      notation: `${aaMatch[1]}${aaMatch[2]}${aaMatch[3]}`,
    };
  }

  const nucMatch = upper.match(/^([ATGC])(\d+)([ATGC])$/);
  if (nucMatch) {
    return {
      type: MUTATION_TYPES.SUBSTITUTION,
      oldBase: nucMatch[1],
      position: parseInt(nucMatch[2]) - 1,
      newBase: nucMatch[3],
      notation: `${nucMatch[1]}${nucMatch[2]}${nucMatch[3]}`,
    };
  }

  const delMatch = upper.match(/^DEL(\d+)(?:-(\d+))?$/);
  if (delMatch) {
    const start = parseInt(delMatch[1]) - 1;
  ...
  if (insMatch) {
    return {
      type: MUTATION_TYPES.INSERTION,
      position: parseInt(insMatch[1]) - 1,
      sequence: insMatch[2],
...
export function designPrimersFromNotation(template, notation, options = {}) {
  const parsed = parseMutationNotation(notation);

  switch (parsed.type) {
    case MUTATION_TYPES.SUBSTITUTION:
      return designSubstitutionPrimers(template, parsed.position, parsed.newBase, options);

    case MUTATION_TYPES.CODON_CHANGE:
      return designCodonChangePrimers(template, parsed.position, parsed.newAA, options);
    ...
    case MUTATION_TYPES.INSERTION:
      return designInsertionPrimers(template, parsed.position, parsed.sequence, options);
...
/**
 * Check if candidate A dominates candidate B (A is better in ALL objectives)
 * Lower values are better for all objectives.
 *
 * @param {Object} a - Candidate A with objective values
 * @param {Object} b - Candidate B with objective values
 * @param {Array} objectives - List of objective keys to compare
...
/**
 * Sort candidates into Pareto fronts using multi-objective optimization.
 *
 * Front 1 contains non-dominated solutions (Pareto optimal)
 * Front 2 contains solutions dominated only by Front 1, etc.
 *
 * Objectives (all minimized):
 * - tmDiff: Tm difference between primers (target: 0)
 * - structurePenalty: Secondary structure ΔG penalty (target: 0)
 * - lengthPenalty: Excess length penalty (target: 0)
 * - gcPenalty: GC content deviation penalty (target: 0)
 *
 * @param {Array} candidates - Array of primer pair candidates
 * @returns {Array} Array of fronts, each containing candidates
 */
function paretoSort(candidates) {
  if (candidates.length === 0) return [];

  // Define objectives to optimize (all should be minimized)
  const objectives = ['tmDiff', 'structurePenalty', 'lengthPenalty', 'gcPenalty'];

  // Calculate objective values for each candidate
  const evaluated = candidates.map(c => ({
    ...c,
    tmDiff: Math.abs(c.forward.tm - c.reverse.tm),
    structurePenalty: Math.max(0, -3 - (c.forward.dg || 0)) + Math.max(0, -3 - (c.reverse.dg || 0)),
    lengthPenalty: Math.max(0, c.forward.length - 20) + Math.max(0, c.reverse.length - 20),
    gcPenalty: Math.abs(c.forward.gc - 0.5) * 10 + Math.abs(c.reverse.gc - 0.5) * 10,
  }));

  const fronts = [];
  const remaining = new Set(evaluated);
  ...
  while (remaining.size > 0) {
    ...
    // Sort front by tmDiff (primary) then by total penalty (secondary)
    front.sort((a, b) => {
      if (a.tmDiff !== b.tmDiff) return a.tmDiff - b.tmDiff;
 ...
 */
function assignTiers(candidates, options = {}) {
  const {
 ...
 *
 * @param {Array} candidates - Array of primer pair candidates
 * @returns {Object|null} Best candidate or null if none available
 */
function selectBestByTier(candidates) {
 ...
 * @param {number} pos - Position to check around
 * @returns {Object} { hasProblems, issues: [] }
 */
function checkSequenceContext(seq, pos) {
  const issues = [];
  const windowSize = 6;
  ...
  if (/(.)\1{3,}/.test(context)) {
    ...
    issues.push({ type: 'homopolymer', pattern: match[0], severity: 'warning' });
  }

  // Check for dinucleotide repeats (ATAT, GCGC, etc.)
  if (/(..)\1{2,}/.test(context)) {
    const match = context.match(/(..)\1{2,}/);
    issues.push({ type: 'dinuc_repeat', pattern: match[0], severity: 'warning' });
  }
 ...
 *
 * This function uses the shared analyzeGQuadruplex from scoring.js.
 ...
 *
 * Canonical G4 motif: G3+N1-7G3+N1-7G3+N1-7G3+
 * Also checks for multiple GGG runs that could form inter-strand G4.
 *
 * @param {string} sequence - DNA sequence to check
 * @returns {Object} { hasG4Motif, hasGGGG, gggCount, gggRuns, severity, message, score }
 */
export function checkGQuadruplexRisk(sequence) {
  // Use the shared analyzeGQuadruplex function from scoring.js
  // This ensures consistent G-quadruplex detection across all modules
 ...
 * @returns {Object} { base, penalty, classification }
 */
export function score3primeTerminalBase(sequence) {
  const seq = sequence.toUpperCase();
  const lastBase = seq.slice(-1);

  // Penalty map (lower is better)
  // C is slightly preferred over G due to G-T wobble pairing risk
  const penalties = {
 ...
 *
 * Performance note: Run this on candidate pairs during selection phase,
 * NOT inside inner loops of candidate generation.
 *
 * @param {Object} fwd - Forward primer { sequence, tm, length, ... }
 * @param {Object} rev - Reverse primer { sequence, tm, length, ... }
 * @param {Object} options - Scoring options
 * @returns {Object} { score, notes, breakdown, fwd, rev }
 */
export function scorePrimerPair(fwd, rev, options = {}) {
  const {
    minTm = 55,
    targetTm = 62,
    minLength = 15,
    includeCompositeScore = true,  // Include unified 0-100 composite score
    temperature = 55,              // Annealing temperature for thermodynamic calculations
  } = options;
  ...
  // Tm Difference (Quadratic Penalty - heavily penalize large differences)
  ...
  // Target Tm Distance (Exponential penalty for being too cold)
  ...
  // Length penalty (mild preference for shorter primers)
  ...
  // A. 3' Terminal Delta G (Check Forward and Reverse)
  ...
  // Ideal range: -6 to -11 kcal/mol
  // > -6: too loose (risk of no initiation)
  // < -12: too sticky (risk of mispriming)
  if (fwdTermDG.dG > -6.0) {
    terminalDGScore += 5;
  ...
  breakdown.terminalDG = {
    fwd: fwdTermDG,
    rev: revTermDG,
    penalty: terminalDGScore,
  };
  ...
  breakdown.terminalBase = {
    fwd: fwdTermBase,
    rev: revTermBase,
    penalty: terminalBasePenalty,
  };
  ...
  // ==========================================================================
  // 3. UNIFIED COMPOSITE SCORE (Shared with standalone and unified designer)
  // ==========================================================================
  // This provides a 0-100 score using the calibrated weights from scoring.js
  // for consistency with other modules in the codebase.

  let compositeScore = null;
  let qualityTier = null;
  let piecewiseScores = null;
  ...
  if (includeCompositeScore) {
    ...
    // Calculate hairpin and homodimer DG
    ...
    // Build piecewise scores object with NaN/Infinity validation
    piecewiseScores = {
      ...
      gQuadruplexFwd: sanitize(fwdG4.score, 1),
      gQuadruplexRev: sanitize(revG4.score, 1),
      // Off-target scoring (0.25 weight - the most important factor)
      offTarget: Math.min(
  ...
  return {
    score,          // Penalty-based score (lower is better, 0 is best)
    notes,
    breakdown,
    fwd,
    rev,
    ...
    // Unified scoring (0-100, higher is better) - consistent with standalone and unified designer
    compositeScore,  // 0-100 score using calibrated weights
    qualityTier,     // 'excellent', 'good', 'acceptable', 'marginal', 'poor'
    piecewiseScores, // Individual feature scores (0-1)
  };
}
 ...
 *
 * This is the main entry point for the combined functionality.
 * It automatically routes to the appropriate design function based on the request type.
 *
 * @param {string} template - Template DNA sequence
 * @param {Object} request - Design request
 * @param {string} request.type - 'substitution', 'codon_change', 'insertion', 'deletion', or 'pcr'
 * @param {Object} options - Design options
 * @returns {Object} Design result with primers, analysis, and protocol
 */
export function designPrimers(template, request, options = {}) {
 ...
 *
 * This function now uses the unified analysis from primerAnalysis.js for consistent
 * scoring across all design modes, while maintaining backward compatibility with
 * mutagenesis-specific features (structure checking with 3' involvement, etc.)
 */
export function analyzePrimerPair(fwdPrimer, revPrimer, template = null, options = {}) {
  ...
  // Mutagenesis-specific secondary structure analysis (with 3' involvement tracking)
  const fwdStructure = checkMutantSecondaryStructure(fwdSeq);
  const revStructure = checkMutantSecondaryStructure(revSeq);

  // Heterodimer analysis (mutagenesis-specific with alignment tracking)
  ...
  // Collect mutagenesis-specific warnings
  const mutagenesisWarnings = [
    ...fwdStructure.warnings.map(w => ({ ...w, primer: 'forward' })),
    ...revStructure.warnings.map(w => ({ ...w, primer: 'reverse' })),
    ...heterodimerResult.warnings.map(w => ({ ...w, primer: 'pair' })),
  ];

  // Use unified analysis for scoring (provides G-quadruplex detection, calibrated composite score)
  const unifiedResult = unifiedAnalyzePrimers(
    ...
    { seq: revSeq, tm: revTm, gc: calculateGC(revSeq) },
    {
      ...
      includeRecommendations: false,  // Use mutagenesis-specific warnings instead
    }
  );

  // Calculate overall quality (combine mutagenesis-specific and unified warnings)
  const criticalWarnings = mutagenesisWarnings.filter(w => w.severity === 'critical').length;
  const warningCount = mutagenesisWarnings.filter(w => w.severity === 'warning').length;
  ...
  return {
    forward: {
      ...
      // NEW: Unified scores for consistent analysis
    ...
    // NEW: Unified analysis additions
 ...
 * Useful for designing primers for multiple mutations in parallel
 */
export function batchDesignMutations(template, mutations, options = {}) {
  const results = [];
 ...
 * Useful for understanding Tm differences
 */
export function compareTmMethods(primer, templateRegion = null) {
  const seq = primer.toUpperCase();
