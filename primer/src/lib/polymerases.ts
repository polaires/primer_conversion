 ...
 * Polymerase ID type
 */
export type PolymeraseId = 'q5' | 'phusion' | 'taq' | 'onetaq' | 'deepvent' | 'kapa';
...
/**
 * Polymerase default settings
 */
export interface PolymeraseDefaults {
  primerConc: number;    // nM
  mgConc: number;        // mM
  annealOffset: number;  // °C added to lower Tm
  maxAnneal: number;     // Maximum annealing temp
  naConc?: number;       // mM (for some polymerases)
}

/**
 * Polymerase configuration
 */
...
/**
 * Annealing temperature calculation result
 */
 ...
 */
export interface TmDetailedResult {
  sequence: string;
  length: number;
  gcContent: number;
  gcPercent: string;
  ...
  polymerase: string;
  method: string;
  options: PolymeraseDefaults;
}

/**
 * Polymerase list item (for UI)
 */
export interface PolymeraseListItem {
  id: PolymeraseId;
  name: string;
  fullName: string;
  manufacturer: string;
...
const BRESLAUER_PARAMS: Record<string, ThermoParamArray> = {
...
// =============================================================================
// Polymerase Definitions
// =============================================================================

/**
 * Polymerase definitions with their specific properties
 */
export const POLYMERASES: Record<PolymeraseId, Polymerase> = {
  q5: {
    id: 'q5',
    name: 'NEB Q5',
 ...
 */
export function calculateTmQ5(sequence: string, options: TmOptions = {}): number {
 ...
 */
export function calculateTmPhusion(sequence: string, options: TmOptions = {}): number {
 ...
 */
export function calculateTmTaq(sequence: string, options: TmOptions = {}): number {
 ...
 */
export function calculateTmOwczarzy(sequence: string, options: TmOptions = {}): number {
  const { primerConc = 500, mgConc = 2.0 } = options;
  const seq = sequence.toUpperCase().replace(/[^ATGC]/g, '');
 ...
 * @param polymeraseId - Polymerase ID (q5, phusion, taq, onetaq, deepvent, kapa)
 * @param options - Optional concentration overrides
 * @returns Melting temperature in °C
 */
export function calculateTm(sequence: string, polymeraseId: PolymeraseId = 'q5', options: TmOptions = {}): number {
 ...
 * @param polymeraseId - Polymerase ID
 * @param options - Optional overrides
 * @returns Annealing temperature result
 */
export function calculateAnnealing(
  primer1: string,
  primer2: string,
  polymeraseId: PolymeraseId = 'q5',
  options: TmOptions = {}
): AnnealingResult {
  const polymerase = POLYMERASES[polymeraseId];
  if (!polymerase) {
  ...
  const mergedOptions = { ...polymerase.defaults, ...options };
  const { annealOffset = 0, maxAnneal = 72 } = mergedOptions;
 ...
 */
export function calculateTmDetailed(
  sequence: string,
  polymeraseId: PolymeraseId = 'q5',
  options: TmOptions = {}
): TmDetailedResult {
  ...
  return {
    ...
    polymerase: polymerase.name,
    method: polymerase.method,
    options: mergedOptions as PolymeraseDefaults,
  };
}
 ...
 */
export function getPolymeraseList(): PolymeraseListItem[] {
 ...
 */
export function getPolymeraseInfo(polymeraseId: PolymeraseId): Polymerase | null {
