 ...
 *
 * @param {string} seq - The template sequence being checked
 * @param {string} checkSeq - The sequence being checked for offtarget binding sites
 * @returns {number[]} A list of binding site counts for primers whose last bp is within that index
 */
export function offTargets(seq, checkSeq) {
  const checkMap = {};
  const mutateMap = { A: "TGC", T: "AGC", G: "ATC", C: "ATG" };

  /**
   * Generate all single-base mutations of a sequence
   * @param {string} s - Input sequence
   * @returns {string[]} Array of mutated sequences including original
   */
  function mutate(s) {
  ...
  // Create the cache
  const cache = new Array(seq.length).fill(0);
 ...
 * @returns {string} The reverse complement of the template sequence
 */
