...
export interface ThresholdConfig {
  tm: ThresholdRange;
  gc: ThresholdRange;
  length: Omit<ThresholdRange, 'hardMax'> & { hardMax: number };
}
...
export interface PairScores {
  ...
  fwdScores: PiecewiseScores;
  revScores: PiecewiseScores;
}
...
export interface PrimerPair {
  ...
  primerScore?: number;
  ampliconScore?: number | null;
  ampliconAnalysis?: any;
}

export interface GenerationOptions {
  numAlternatives?: number;
  maxCandidatesPerDirection?: number;
  topCandidatesForPairing?: number;
  annealingTemp?: number;
  maxTmDiff?: number;
  earlyRejectTmDiff?: number;
  minLength?: number;
  maxLength?: number;
  addLen?: number;
}
...
export interface EnhancedGenerationOptions extends GenerationOptions {
  ...
  fwdSearchStart?: number;
  fwdSearchEnd?: number | null;
  revSearchStart?: number | null;
  revSearchEnd?: number | null;
  windowStep?: number;
  minAmpliconLength?: number;
  maxAmpliconLength?: number;
  targetAmpliconLength?: number | null;
  application?: 'pcr' | 'qpcr' | 'sequencing';
  includeAmpliconAnalysis?: boolean;
  ampliconScoreWeight?: number;
}
 ...
 *
 * @param primer - Primer with tm, gc, length properties
 * @returns Quick score 0-1
 */
export function calculateQuickScore(primer: PrimerCandidate): number {
 ...
 *
 * @param primer - Primer object with seq, tm, gc, dg
 * @returns Medium score 0-1 (average of piecewise scores)
 */
export function calculateMediumScore(primer: PrimerCandidate): number {
 ...
 *
 * @param primer - Primer object with seq, tm, gc, dg
 * @returns Piecewise scores
 */
export function calculatePrimerPiecewiseScores(primer: PrimerCandidate): PiecewiseScores {
 ...
 * @param annealingTemp - Annealing temperature for dimer calculations
 * @returns Pair scores and composite
 */
export function calculatePairScores(
  fwd: PrimerCandidate,
  rev: PrimerCandidate,
  annealingTemp: number = 55
): PairScores {
 ...
 * @returns Array of primer candidates with position info
 */
function generateCandidates(
  ...
  for (let start = 0; start < Math.min(maxStart, seq.length - minLength); start++) {
    for (let len = minLength; len <= maxLength; len++) {
      ...
      const candidate: PrimerCandidate = {
        seq: primerSeq,
        length: len,
        tm: primerTm,
        gc: primerGc,
...
// ============================================================================
// Main Alternative Generation Functions
// ============================================================================
 ...
 *
 * @param template - Template sequence for the region to amplify
 * @param options - Generation options
 * @returns Array of primer pair alternatives with labels
 */
export function generateAlternativesUnified(
  template: string,
  options: GenerationOptions = {}
): PrimerPair[] {
  const {
  ...
  const diverseRanges: DistanceRanges = {
    maxPositionDiff: 20,
  ...
  const paretoObjectives = [
    (p: any) => p.compositeScore,           // Higher score better
    (p: any) => -p.tmDiff,                  // Lower Tm diff better
    (p: any) => p.heterodimerDG,            // Higher (less negative) better
    (p: any) => -p.ampliconLength,          // Shorter amplicon as option
  ];

  const diversePairRanges: DistanceRanges = {
 ...
 *
 * @param pair - Primer pair
 * @returns Tier label
 */
export function assignTier(pair: any): string {
 ...
 * @param options - Generation options
 * @returns Array of primer pair alternatives with comprehensive scoring
 */
export function generateAlternativesEnhanced(
  template: string,
  ...
  let fwdCandidates: PrimerCandidate[], revCandidates: PrimerCandidate[];

  if (useSlidingWindow) {
    // TODO: Implement generateSlidingWindowCandidates
    // For now, fall back to fixed-end generation
    const fwdRegion = seq.slice(0, LEN_MAX + 10);
    ...
    for (const rev of topRev) {
      ...
      if (includeAmpliconAnalysis) {
        ...
        if (ampliconSeq.length > 0) {
          // TODO: Implement scoreAmplicon function
          // For now, use a simple placeholder
          ampliconAnalysis = { score: 0.8, lengthScore: 0.9 };
  ...
  const paretoObjectives = [
    ...
    (p: any) => -p.tmDiff,                  // Lower Tm diff better
    (p: any) => p.heterodimerDG,            // Higher (less negative) better
    (p: any) => p.ampliconScore ?? 80,      // Higher amplicon score better
  ];

  // Add amplicon length objective based on target
  if (targetAmpliconLength !== null) {
    paretoObjectives.push((p: any) => -Math.abs(p.ampliconLength - targetAmpliconLength));
  } else {
    paretoObjectives.push((p: any) => -p.ampliconLength);  // Shorter as option
  }
 ...
 */
function identifyStrengthsEnhanced(candidate: any, allCandidates: any[]): string[] {
 ...
 */
const ENHANCED_LABEL_ICONS: Record<string, string> = {
 ...
 */
function generateLabelEnhanced(strengths: string[], pair: any): LabelInfo | null {
 ...
 */
function generateExplanationEnhanced(candidate: any, strengths: string[], reference: any): string | null {
  if (strengths.includes('cleanAmplicon')) {
