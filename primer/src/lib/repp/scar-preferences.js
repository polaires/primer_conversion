 ...
 * @param {string} contextBefore - Optional 2bp before overhang for frame 2 check
 * @returns {Object} Stop codon analysis
 */
export function checkStopCodons(overhang, contextBefore = '') {
 ...
 *
 * @param {string} overhang - 4bp overhang sequence
 * @param {string} contextBefore - 2bp before overhang (for frame context)
 * @param {string} contextAfter - 2bp after overhang (for frame context)
 * @returns {Object} Translation in all frames
 */
export function translateOverhang(overhang, contextBefore = '', contextAfter = '') {
  const oh = overhang.toUpperCase();
  const before = contextBefore.toUpperCase();
 ...
 *
 * @param {string} overhang - 4bp overhang sequence
 * @param {string} context - Context type ('coding', 'linker', 'nonCoding', 'startCodon', 'stopCodon')
 * @returns {Object} Score and recommendations
 */
export function scoreScarSequence(overhang, context = 'nonCoding') {
 ...
 */
export function scoreScarSet(overhangs, context = 'nonCoding') {
 ...
 *
 * @param {string[]} candidates - Array of candidate overhangs
 * @param {string} context - Context type
 * @returns {Object} Best candidate analysis
 */
export function findBestScarCandidate(candidates, context = 'nonCoding') {
 ...
 *
 * @param {Object} assemblyInfo - Information about the assembly
 * @returns {Object} Context recommendations
 */
export function getContextRecommendations(assemblyInfo) {
  const {
  ...
  const recommendations = junctionPositions.map((pos, idx) => {
    let context = 'nonCoding';
