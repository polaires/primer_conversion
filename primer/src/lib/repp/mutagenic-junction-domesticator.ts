...
interface MutagenicJunctionConfig {
  homologyLength: number;
  minHomologyLength: number;
  maxHomologyLength: number;
  searchRadius: number;
  minDistanceFromSiteEdge: number;
  preferHighFidelityOverhangs: boolean;
  minOverhangScore: number;
  preferWobblePosition: boolean;
  avoidRareCodons: boolean;
...
interface JunctionCandidate {
  ...
  bestMutation: MutationCandidate;
...
interface PrimerDesignResult {
  fragment1: {
    reversePrimer: PrimerDesign;
    note: string;
  };
  fragment2: {
    forwardPrimer: PrimerDesign;
    note: string;
  };
  mutation: MutationInfo;
}

interface MutagenicJunctionResult {
  success: boolean;
  site: InternalSite;
  junctionPosition?: number;
  overhang?: string;
  ...
  alternativeMutations?: MutationCandidate[];
...
interface MutagenicJunctionOptions {
  frame?: number;
...
interface AllJunctionsOptions {
  frame?: number;
  organism?: 'ecoli' | 'yeast';
  existingOverhangs?: string[];
}

interface AllJunctionsResult {
  success: boolean;
  needsDomestication: boolean;
  totalSites?: number;
  sites?: InternalSite[];
  junctions?: MutagenicJunctionResult[];
...
interface SiteAnalysis {
  ...
  nearestJunction: NearestJunction | null;
  strategies: {
    directPrimerMutation: DomesticationStrategy;
    mutagenicJunction: DomesticationStrategy;
  };
...
interface AlternativeEnzymes {
  alternatives: AlternativeEnzyme[];
  hasCompatible: boolean;
  best: AlternativeEnzyme;
}

interface StrategySelectionOptions {
  frame?: number;
  userJunctionPositions?: number[];
...
interface StrategySelectionResult {
  ...
  alternativeEnzymes?: AlternativeEnzymes;
  summary?: {
    directPrimerMutation: number;
    mutagenicJunction: number;
    failed: number;
    additionalFragments: number;
  };
...
type CodonUsageTable = Record<string, number>;

// ============================================================================
// CONFIGURATION
// ============================================================================

export const MUTAGENIC_JUNCTION_CONFIG: MutagenicJunctionConfig = {
 ...
 */
export function designMutagenicJunction(
  sequence: string,
  site: InternalSite,
  enzyme: string = 'BsaI',
  options: MutagenicJunctionOptions = {}
  ...
  for (let junctionPos = searchStart; junctionPos <= searchEnd; junctionPos++) {
    ...
    const siteBasesInFrag1Primer: Array<{ position: number; indexInSite: number }> = [];
    const siteBasesInFrag2Primer: Array<{ position: number; indexInSite: number }> = [];
...
function findMutationsForCoveredBases(
  sequence: string,
  site: InternalSite,
  frag1Coverage: Array<{ position: number; indexInSite: number }>,
  frag2Coverage: Array<{ position: number; indexInSite: number }>,
  frame: number,
  enzyme: string,
  ...
  const uniquePositions = new Map<number, { position: number; indexInSite: number; inFragment1Primer: boolean; inFragment2Primer: boolean }>();
 ...
 */
function scoreOverhangQuality(overhang: string, existingOverhangs: string[], enzyme: string): OverhangScore {
  let score = 70;
  const issues: string[] = [];
 ...
 */
function designMutagenicPrimers(
  sequence: string,
  junctionPos: number,
  overhang: string,
  mutation: MutationCandidate,
  ...
  const fragment2ForwardPrimer: PrimerDesign = {
  ...
  const fragment1ReversePrimer: PrimerDesign = {
 ...
 */
export function designAllMutagenicJunctions(
  sequence: string,
  enzyme: string = 'BsaI',
  options: AllJunctionsOptions = {}
): AllJunctionsResult {
  const {
    frame = 0,
    organism = 'ecoli',
  ...
  const junctions: MutagenicJunctionResult[] = [];
 ...
 */
export function selectDomesticationStrategy(
  sequence: string,
  enzyme: string = 'BsaI',
 ...
 */
function calculateIdealJunctions(seqLength: number, numFragments: number): number[] {
 ...
 */
function findNearestJunction(position: number, junctions: number[]): NearestJunction | null {
 ...
 */
function checkAlternativeEnzymes(sequence: string, currentEnzyme: string): AlternativeEnzymes {
  const results: AlternativeEnzyme[] = [];
