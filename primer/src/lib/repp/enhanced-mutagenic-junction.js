...
import {
  ...
  findOptimalOverhangSetExhaustive,
} from './goldengate.js';
...
export const ENHANCED_JUNCTION_CONFIG = {
  ...
  searchRadius: 50,              // Search ±50bp around site (was 30bp)
  extendedSearchRadius: 80,      // Extended search for difficult sites
  ...
  targetTm: 62,                  // Target Tm for homology region (NEB recommends 60-65°C)
  tmTolerance: 5,                // Acceptable Tm deviation
  useAdaptiveHomology: true,     // Auto-adjust homology length to hit target Tm

  // Overhang selection (ENHANCED with NEB data)
  preferHighFidelityOverhangs: true,
  minOverhangFidelity: 0.95,     // Minimum single overhang fidelity
  targetAssemblyFidelity: 0.98,  // Target overall assembly fidelity
  ...
  weights: {
    overhangFidelity: 0.35,      // NEB experimental fidelity (highest weight)
    mutationQuality: 0.25,       // Codon optimization
    primerQuality: 0.25,         // Primer thermodynamics
    positionOptimality: 0.15,    // Reading frame, distance from site
  },
 ...
 *
 * This is the main entry point for enhanced domestication. It:
 * 1. Searches wider range around internal sites
 * 2. Uses NEB experimental fidelity for overhang selection
 * 3. Scores primer quality using thermodynamic analysis
 * 4. Considers global overhang compatibility
 * 5. Respects reading frame when possible
 *
 ...
 * @returns {Object} Enhanced mutagenic junction design
 */
export function designEnhancedMutagenicJunction(sequence, site, enzyme = 'BsaI', options = {}) {
  const {
    frame = 0,
    organism = 'ecoli',
    existingOverhangs = [],
 ...
 */
function enumerateJunctionCandidates(sequence, site, enzyme, options) {
  const {
    searchRadius,
    frame,
    codonUsage,
 ...
 */
function calculatePrimerCoverage(junctionPos, siteStart, siteEnd, seqLen, homologyLen) {
  const overhangLen = 4;
 ...
 * Calculate position optimality (prefer keeping codons intact)
 */
function calculatePositionOptimality(junctionPos, siteStart, siteEnd, frame, overhangLen) {
  let score = 100;
 ...
 */
function findSilentMutationsForCoverage(sequence, site, coverage, frame, enzyme, codonUsage) {
  const mutations = [];
  const recognition = site.sequence;
  const siteStart = site.position;
  const enz = GOLDEN_GATE_ENZYMES[enzyme];
 ...
 */
function scoreMutation(originalCodon, newCodon, posInCodon, posInSite, siteLen, codonUsage) {
  const breakdown = {};
  let score = 80;
 ...
 */
function scoreJunctionCandidates(candidates, sequence, enzyme, options) {
  const {
    existingOverhangs,
    targetTm,
    hasExperimentalData,
 ...
 */
export function scoreOverhangFidelity(overhang, existingOverhangs, enzyme, hasExperimentalData, enzymeData) {
 ...
 */
function scoreOverhangHeuristic(overhang) {
  let score = 70;
  const issues = [];
  ...
  if (isPalindrome(overhang)) {
    score -= 40;
    issues.push('palindrome');
  }

  // Homopolymer check
  if (/^(.)\1{3}$/.test(overhang)) {
    score -= 30;
    issues.push('homopolymer');
 ...
 */
export function scorePrimerQuality(sequence, junctionPos, coverage, mutation, targetTm) {
  const overhangLen = 4;
 ...
 */
function analyzeHomologyRegion(region, isForward, targetTm) {
  if (!region || region.length < 10) {
    return { score: 30, tm: 0, gc: 0, issues: ['Too short'] };
  }
 ...
 */
function calculateAdaptiveHomologyLength(sequence, startPos, direction, targetTm, config) {
  const { min, max, optimal } = config;

  // Direction: 'forward' = extend from startPos rightward
  //            'reverse' = extend from startPos leftward
  let bestLen = optimal;
 ...
 */
function designOptimizedMutagenicPrimers(sequence, candidate, enzyme, options) {
  ...
  // Calculate adaptive homology length based on Tm (STATE-OF-THE-ART)
  ...
  if (ENHANCED_JUNCTION_CONFIG.useAdaptiveHomology) {
    // Calculate optimal length for each primer to hit target Tm
  ...
  // Forward primer (Fragment 2): 5'-[Flanking]-[Recognition]-[Spacer]-[Overhang]-[Homology]-3'
  const fragment2ForwardPrimer = {
    fullSequence: flanking + recognition + spacer + candidate.overhang + frag2Homology,
    components: {
      flanking,
      recognition,
      spacer,
  ...
  // Reverse primer (Fragment 1): 5'-[Flanking]-[Recognition]-[Spacer]-[RC(Overhang)]-[RC(Homology)]-3'
  const fragment1ReversePrimer = {
    fullSequence: flanking + recognition + spacer +
  ...
  const fwd = fragment2ForwardPrimer;
  const rev = fragment1ReversePrimer;

  let fwdTm = 0, revTm = 0;
  try {
    fwdTm = calculateTmQ5(fwd.components.homology);
    revTm = calculateTmQ5(rev.components.homology);
  } catch (e) {}

  return {
    fragment1: {
      reversePrimer: {
        sequence: rev.fullSequence,
        ...rev,
        tm: revTm,
        length: rev.fullSequence.length,
    ...
    fragment2: {
      forwardPrimer: {
        sequence: fwd.fullSequence,
        ...fwd,
        tm: fwdTm,
        length: fwd.fullSequence.length,
 ...
 */
function validateJunctionDesign(candidate, primerDesign, existingOverhangs, enzyme) {
  const warnings = [];
  const errors = [];
 ...
 */
function isPalindrome(overhang) {
 ...
 */
function generateJunctionMessage(candidate, primerDesign) {
 ...
 */
export function designAllEnhancedJunctions(sequence, enzyme = 'BsaI', options = {}) {
  const {
    frame = 0,
    organism = 'ecoli',
    existingOverhangs = [],
 ...
 */
function optimizeGlobalOverhangSelection(sequence, sites, enzyme, options) {
 ...
 */
function exhaustiveOverhangSearch(siteCandidates, sequence, enzyme, options) {
  ...
  function enumerate(siteIndex, currentOverhangs, currentJunctions) {
  ...
  const junctions = bestCombination.junctions.map(({ site, candidate }) => {
    return designEnhancedMutagenicJunction(sequence, site, enzyme, {
      frame,
      organism,
      existingOverhangs: [...existingOverhangs, ...bestCombination.overhangs.filter(oh => oh !== candidate.overhang)],
    });
  });

  return {
    junctions,
    allOverhangs: [...existingOverhangs, ...bestCombination.overhangs],
 ...
 */
function greedyOverhangSearch(siteCandidates, sequence, enzyme, options) {
  const { existingOverhangs, frame, organism } = options;

  const selectedOverhangs = [];
  const selectedJunctions = [];
 ...
 */
function generateBatchMessage(junctions, failedSites, totalSites) {
  const parts = [];

  if (junctions.length > 0) {
    const avgQuality = junctions.reduce((sum, j) => sum + j.quality.overall, 0) / junctions.length;
...
// Note: ENHANCED_JUNCTION_CONFIG is already exported inline above
