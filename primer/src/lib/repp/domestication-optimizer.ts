...
export function generateDomesticationReport(
  ...
  // Protein verification (imported from silent-mutation-domesticator if available)
  if (domesticationResult.originalSequence && domesticationResult.domesticatedSequence) {
    // Translate and compare proteins
    const translateSequence = (seq: string, fr: number): string => {
      const CODON_TO_AA_MAP: Record<string, string> = {
      ...
      for (let i = fr; i + 3 <= seq.length; i += 3) {
        ...
        const aa = CODON_TO_AA_MAP[codon] || '?';
        protein.push(aa);
        if (aa === '*') break;
      }
      return protein.join('');
    ...
    const originalProtein = translateSequence(domesticationResult.originalSequence, frame);
    const domesticatedProtein = translateSequence(domesticationResult.domesticatedSequence, frame);

    report.proteinVerification = {
      identical: originalProtein === domesticatedProtein,
      originalProtein,
      domesticatedProtein,
