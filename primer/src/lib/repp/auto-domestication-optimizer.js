...
import { GOLDEN_GATE_ENZYMES, findInternalSites } from './goldengate.js';
...
import { scoreFusionSiteComposite } from './fusion-site-scorer.js';
 ...
 *
 * @param {Array} sites - Array of site objects with position property
 * @param {number} minDistance - Minimum allowed distance between sites (default: 50bp)
 * @returns {Object} Analysis of adjacent site issues
 */
export function detectAdjacentSites(sites, minDistance = DOMESTICATION_DEFAULTS.minSiteDistance) {
  if (!sites || sites.length < 2) {
    return { hasAdjacentSites: false, adjacentPairs: [], minDistanceFound: Infinity };
  }
 ...
 * @param {string} currentEnzyme - Current enzyme being used
 * @param {Object} options - Options for recommendation
 * @returns {Array} Ranked list of enzymes with site counts (current enzyme marked)
 */
export function recommendAlternativeEnzymes(sequence, currentEnzyme = 'BsaI', options = {}) {
  ...
  alternatives.sort((a, b) => {
    ...
    if (a.hasLigationData !== b.hasLigationData) return b.hasLigationData - a.hasLigationData;
    return 0;
  });
 ...
 * @param {Array} junctions - Array of junction positions
 * @param {Object} constraints - Size constraints { minFragmentSize, maxFragmentSize }
 * @returns {Object} Validation result with any violations
 */
export function validateFragmentSizes(sequence, junctions, constraints = {}) {
  const {
  ...
  if (!junctions || junctions.length === 0) {
    return { valid: true, fragments: [{ start: 0, end: sequence.length, size: sequence.length }], violations: [] };
  }

  // Sort junction positions
  const positions = [0, ...junctions.map(j => typeof j === 'number' ? j : j.position).sort((a, b) => a - b), sequence.length];
  const fragments = [];
  const violations = [];
 ...
 * @param {Object} options - Analysis options including minFragmentSize
 * @returns {Object} Analysis result with sites and domestication options
 */
export function analyzeForDomestication(sequence, enzyme = 'BsaI', options = {}) {
  const {
    minFragmentSize = DOMESTICATION_DEFAULTS.minFragmentSize,
  ...
  if (invalidOptions.length > 0) {
    ...
    return {
      ...
      error: {
        ...
        invalidSites: invalidOptions.map(opt => ({
 ...
 * @returns {Object} Junction options for domesticating this site
 */
export function findDomesticationJunctions(sequence, site, enzyme = 'BsaI') {
  const enz = GOLDEN_GATE_ENZYMES[enzyme];
  if (!enz) {
    throw new Error(`Unknown enzyme: ${enzyme}`);
  }
  ...
  // A junction position is where the 4bp overhang will be created
  const candidates = [];

  // Scan positions where placing a junction would break the recognition site
  // The junction creates an overhang at positions [pos, pos+4]
  // We need this overhang to overlap with the recognition site

  // For BsaI: GGTCTC with cut at N+1 leaving 4bp overhang
  // If recognition is at pos 100-105, we need junction overhangs
  // that span into positions 100-105 to break the pattern

  // Calculate valid junction range
  // Junction at position P creates overhang at sequence[P:P+4]
  // To break recognition at [siteStart, siteEnd], overhang must overlap
 ...
 *
 * @param {number} junctionPos - Position where junction overhang starts
 * @param {number} overhangLen - Length of the overhang (typically 4bp)
 * @param {number} siteStart - Start position of recognition site
 * @param {number} siteEnd - End position of recognition site
 * @param {string} recognition - Recognition sequence
 * @param {boolean} isReverse - Whether this is a reverse orientation site
 * @returns {boolean} True if junction breaks the site
 */
function doesJunctionBreakSite(junctionPos, overhangLen, siteStart, siteEnd, recognition, isReverse) {
  ...
  // For both forward and reverse sites, we need the overhang to physically
  // disrupt the recognition sequence. The key is that the overhang must
  // overlap with at least one internal base of the recognition sequence.
 ...
 */
function scoreOverhangForDomestication(overhang, enzyme, sequence, position) {
 ...
 * This happens if the scar sequence (overhang region in final product)
 * contains or creates the recognition pattern
 *
 * When fragments ligate:
 * - Fragment 1 ends at junction position with overhang
 * - Fragment 2 starts with the same overhang
 * - The scar in the final product is: [upstream context][overhang][downstream context]
 *
 * We must check if this scar region contains the recognition site.
 *
 * @param {string} sequence - Full DNA sequence
 * @param {number} junctionPos - Position where overhang starts
 * @param {number} overhangLen - Length of overhang (4 for BsaI, 3 for SapI)
 * @param {string} recognition - Recognition sequence to check for
 * @param {string} enzyme - Enzyme name for additional checks
 * @returns {boolean} True if junction would recreate the recognition site
 */
function wouldRecreateRecognitionSite(sequence, junctionPos, overhangLen, recognition, enzyme) {
  const seq = sequence.toUpperCase();
  const recognitionRC = reverseComplement(recognition);

  // When we place a junction to break an internal site, the context around the junction
  // WILL contain the recognition site (that's the site we're trying to break!).
  // The question is: will the SCAR in the final assembled product create a PROBLEMATIC pattern?
  //
  // Key insight: After domestication, the assembled product has the SAME sequence as the original.
  // So the original site will "exist" in the final product. BUT that's OK because:
  // 1. The fragments were cut and ligated at the PRIMER-ADDED sites (at fragment ends)
  // 2. The internal site was broken during fragment creation
  // 3. The final product is just DNA - the site existing there doesn't matter
  //
  // What we DO need to check for is:
  // 1. The overhang being a palindrome of recognition (causes issues during ligation)
  // 2. Very low quality overhangs (handled separately by scoring)
  //
  // The original concern about "recreating" sites was about edge cases where
  // ligation could create NEW sites at the scar that weren't in the original,
  // but this doesn't apply when breaking an existing site.
  ...
  // This would be very bad - the overhang would get cut during assembly
  if (overhang === recognition.slice(0, overhangLen) ||
      overhang === recognitionRC.slice(0, overhangLen)) {
    // Only problematic if the overhang is at the START of the recognition pattern
    // because that's where cutting would occur
    return true;
  }

  // Check 2: Would the primer-added sequence + overhang form a recognition site?
  // For BsaI, primers add: ...NGGTCTCN[overhang]...
  // If overhang starts with the end of the recognition, that could be problematic
  // E.g., if overhang is "TCNN", primer adds "GGTCTC N" + "TCNN" = "GGTCTCNTCNN"
  // This doesn't create a new internal site.

  // Check 3: Could the scar region form a recognition site with slightly different context?
  // This is an edge case for sequences where the junction is placed OUTSIDE the
  // existing site but nearby. In that case, we need to check if scar creates NEW site.
  // But for domestication, we specifically place junctions INSIDE existing sites.
 ...
 *
 * This is the main entry point for auto-domestication. It:
 * 1. Finds internal sites
 * 2. Determines required domestication junctions
 * 3. Merges these with user-requested fragment junctions
 * 4. Runs optimization considering all constraints
 *
 * @param {string} sequence - DNA sequence
 * @param {number} requestedFragments - Number of fragments user wants
 * @param {string} enzyme - Enzyme name
 * @param {Object} options - Optimization options
 * @returns {Object} Optimization result with domestication info
 */
export function optimizeWithDomestication(sequence, requestedFragments, enzyme = 'BsaI', options = {}) {
  const {
    weights = {},
    constraints = {},
    bioContext = {},
  ...
  const totalJunctions = totalFragments - 1; // For circular assembly, this is totalFragments

  // Step 3: Create required junction positions from domestication
  const requiredJunctions = domesticationJunctions.map(opt => ({
  ...
  // excluding regions around required junctions
  const excludeZones = requiredJunctions.map(j => ({
 ...
 */
function generateUserJunctionTargets(seqLength, numJunctions, excludeZones, constraints = {}) {
 ...
 */
export function getDomesticationSummary(sequence, enzyme = 'BsaI') {
  ...
  const siteDescriptions = analysis.sites.map((site, i) => {
    const option = analysis.domesticationOptions[i];
    return {
 ...
 */
export function validateDomesticationOverhangs(domesticationOverhangs, otherOverhangs, enzyme) {
 ...
 *
 * This considers ALL overhangs together (domestication + user-defined) to avoid
 * cross-ligation issues and maximize fidelity.
 *
 * @param {Array} domesticationOptions - Array of domestication options with candidates
 * @param {Array} existingOverhangs - Pre-existing overhangs in the assembly
 * @param {string} enzyme - Enzyme name for fidelity calculation
 * @param {Object} options - Optimization options
 * @returns {Object} Optimized selection with fidelity score
 */
export function optimizeGlobalOverhangSet(domesticationOptions, existingOverhangs = [], enzyme = 'BsaI', options = {}) {
  const {
    maxCombinations = 10000, // Limit search space for performance
    minFidelity = 0.85,
  } = options;
 ...
 */
function searchAllCombinations(siteCandidates, existingOverhangs, enzyme) {
  ...
  const evaluate = () => {
    const selection = indices.map((idx, siteIdx) => siteCandidates[siteIdx][idx]);
 ...
 */
function greedyOptimizeOverhangs(siteCandidates, existingOverhangs, enzyme) {
  // Start with highest-scoring candidate for each site
  let selection = siteCandidates.map(candidates => candidates[0]);
  ...
  while (improved && iterations < maxIterations) {
    ...
    for (let siteIdx = 0; siteIdx < siteCandidates.length; siteIdx++) {
      ...
      for (const candidate of siteCandidates[siteIdx]) {
        if (candidate === currentCandidate) continue;

        // Create new selection with this candidate
        const newSelection = [...selection];
        newSelection[siteIdx] = candidate;
 ...
 * domestication group. This allows handling edge cases where sites are
 * close but can still be managed with careful junction placement.
 *
 * @param {Array} sites - Array of internal site objects
 * @param {string} sequence - Full DNA sequence
 * @param {Object} options - Merging options
 * @returns {Object} Merged site groups with recommended strategy
 */
export function mergeAdjacentSites(sites, sequence, options = {}) {
  const {
    mergeDistance = 100, // Sites within this distance can be merged
    minFragmentSize = 50,
  } = options;
  ...
  if (sites.length === 1) {
    return {
      groups: [{ sites: [sites[0]], type: 'single', canHandle: true }],
      needsMerging: false,
    };
  }
 ...
 */
function analyzeGroup(sites, sequence, minFragmentSize) {
  if (sites.length === 1) {
    return {
      sites,
      type: 'single',
 ...
 *
 * This function performs comprehensive validation after domestication:
 * 1. Verifies no recognition sites are recreated at junction scars
 * 2. Validates all fragment sizes meet minimum requirements
 * 3. Checks overhang uniqueness and fidelity
 * 4. Simulates assembly to verify correct final sequence
 *
 * @param {Array} fragments - Array of fragment objects with seq, overhang properties
 * @param {string} enzyme - Enzyme name (e.g., 'BsaI')
 * @param {Object} options - Validation options
 * @returns {Object} Validation result with issues and recommendations
 */
export function validatePostDomestication(fragments, enzyme = 'BsaI', options = {}) {
  const {
    minFragmentSize = DOMESTICATION_DEFAULTS.minFragmentSize,
    expectedSequence = null, // Optional: original sequence for assembly verification
    isCircular = false,
  } = options;
  ...
  if (!enz) {
    return {
      isValid: false,
      issues: [{ type: 'error', message: `Unknown enzyme: ${enzyme}` }],
      warnings: [],
      passed: [],
    };
  }
  ...
  for (let i = 0; i < fragments.length - 1; i++) {
    ...
    // Note: overhang appears once in assembled product (not duplicated)
