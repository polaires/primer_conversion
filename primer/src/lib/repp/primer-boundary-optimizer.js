...
export const BOUNDARY_OPTIMIZER_DEFAULTS = {
 ...
 * @property {string} seq - DNA sequence
 * @property {number} [start] - Start position in original sequence (optional)
 * @property {number} [end] - End position in original sequence (optional)
 */

/**
 * @typedef {Object} Junction
 * @property {number} index - Index of the junction (between fragments[index] and fragments[index+1])
 * @property {number} position - Absolute position in the concatenated sequence
 * @property {string} overhang - 4bp overhang at this junction
 * @property {Object} leftPrimer - Primer binding on left fragment (reverse primer)
 * @property {Object} rightPrimer - Primer binding on right fragment (forward primer)
 */

/**
 * @typedef {Object} BoundaryOptimizationResult
 * @property {boolean} success - Whether optimization succeeded
 * @property {number} originalPosition - Original boundary position
 * @property {number} optimizedPosition - New boundary position
 * @property {number} shift - How much the boundary shifted (negative = left, positive = right)
 * @property {string} direction - 'left', 'right', or 'unchanged'
 * @property {Object} beforePrimers - Primer quality before optimization
 * @property {Object} afterPrimers - Primer quality after optimization
 * @property {number} improvement - Score improvement (0-1)
 ...
 * @param {string} homology - The primer binding region (homology sequence)
 * @param {Object} options - Scoring options
 * @returns {Object} Quality metrics
 */
function scorePrimerBindingRegion(homology, options = {}) {
 ...
 * @param {number} position - Position where overhang starts
 * @returns {string} 4bp overhang
 */
function getOverhangAt(sequence, position) {
 ...
 */
function validateOverhang(overhang, options = {}) {
 ...
 *
 * @param {string} leftSeq - Sequence of the left fragment
 * @param {string} rightSeq - Sequence of the right fragment
 * @param {Object} options - Optimization options
 * @returns {BoundaryOptimizationResult} Optimization result
 */
export function optimizeJunctionBoundary(leftSeq, rightSeq, options = {}) {
 ...
 *
 * @param {Fragment[]} fragments - Array of fragments with sequences
 * @param {Object} options - Optimization options
 * @returns {Object} Full optimization result
 */
export function optimizeAssemblyBoundaries(fragments, options = {}) {
  const config = { ...BOUNDARY_OPTIMIZER_DEFAULTS, ...options };
  ...
  for (let i = 0; i < fragments.length - 1; i++) {
    ...
    if (!leftSeq || !rightSeq) {
      results.push({
        ...
        shift: 0,
        direction: 'unchanged',
        leftFragmentId: leftFrag?.id || `Fragment_${i + 1}`,
        rightFragmentId: rightFrag?.id || `Fragment_${i + 2}`,
      });
      continue;
    }
    ...
    junctionResult.junctionIndex = i;
    junctionResult.leftFragmentId = leftFrag.id || `Fragment_${i + 1}`;
    junctionResult.rightFragmentId = rightFrag.id || `Fragment_${i + 2}`;
 ...
 *
 * @param {Fragment[]} fragments - Array of fragments
 * @param {Object} options - Options
 * @returns {Object} Quick assessment
 */
export function assessBoundaryOptimizationPotential(fragments, options = {}) {
  const config = { ...BOUNDARY_OPTIMIZER_DEFAULTS, ...options };
  ...
  for (let i = 0; i < fragments.length - 1; i++) {
    ...
    if (leftScore.quality === 'poor' || leftScore.quality === 'acceptable') {
      issues.push({
        junction: i,
        side: 'left',
        fragmentId: leftFrag.id || `Fragment_${i + 1}`,
    ...
    if (rightScore.quality === 'poor' || rightScore.quality === 'acceptable') {
      issues.push({
        junction: i,
        side: 'right',
        fragmentId: rightFrag.id || `Fragment_${i + 2}`,
