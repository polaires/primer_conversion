 ...
 * @returns {Object} Domestication plan requiring user approval
 */
export function createDomesticationPlan(sequence, enzyme = 'BsaI', options = {}) {
  const {
    frame = null,
    organism = 'ecoli',
  ...
  const confirmedFrame = frameStep.confirmedFrame ?? frameStep.recommendedFrame;
 ...
 *
 * @param {Object} plan - Approved domestication plan
 * @param {Object} userSelections - User's selections for each action
 * @returns {Object} Domestication result
 */
export function executeDomesticationPlan(plan, userSelections = {}) {
  const {
    sequence,
    enzyme,
    selectedStrategy,
  ...
  const frame = userSelections.frame ?? plan.steps.find(s => s.step === 'FRAME_DETECTION')?.confirmedFrame;
  ...
  for (const siteOption of mutationStep.siteOptions) {
    ...
    const selectedOption = userSelections[siteKey] ?? siteOption.recommended;

    if (selectedOption && selectedOption.type === 'silent_mutation') {
    ...
    } else if (selectedOption && selectedOption.type === 'mutagenic_junction') {
 ...
 * @param {Array} junctions - Junction definitions
 * @param {string} enzyme - Enzyme name
 * @param {number} frame - Reading frame
 * @returns {Object} Fragment and primer information
 */
function generateMutagenicFragments(sequence, junctions, enzyme, frame) {
 ...
 */
function generateDefaultPrimer(sequence, position, direction, enzyme) {
 ...
 */
function applyMutationsToPrimer(primer, mutations, direction) {
 ...
 */
function verifyMutagenicJunctions(originalSeq, fragmentResult, junctions, frame, enzyme) {
  const issues = [];
  const checks = [];
 ...
 * @param {string} enzyme - Enzyme name
 * @param {Object} internalSites - Result from findInternalSites
 * @param {number|null} frame - Reading frame (if known)
 * @param {string} organism - Target organism
 * @param {Array} existingOverhangs - Existing overhangs to avoid conflicts
 * @returns {Object} Strategy options step
 */
function generateStrategyOptions(sequence, enzyme, internalSites, frame, organism, existingOverhangs) {
  const step = {
 ...
 */
function handleFrameDetection(sequence, providedFrame, sites, organism) {
  const step = {
 ...
 */
function handleAdjacentSites(sequence, sites, enzyme, frame, organism) {
  const step = {
  ...
  for (const pair of adjacentCheck.adjacentPairs) {
    ...
    if (sharedMutation) {
      pairResolutions.push({
        strategy: ENHANCED_CONFIG.adjacentSiteStrategies.OVERLAPPING_MUTATION,
    ...
    if (singleJunction) {
      pairResolutions.push({
        strategy: ENHANCED_CONFIG.adjacentSiteStrategies.SINGLE_JUNCTION,
    ...
    if (compatibleAlt) {
      pairResolutions.push({
        strategy: ENHANCED_CONFIG.adjacentSiteStrategies.ALTERNATIVE_ENZYME,
 ...
 *
 * Options are ordered by strategy preference:
 * 1. Mutagenic Junction (PRIMARY) - one-pot Golden Gate compatible
 * 2. Silent Mutation (ALTERNATIVE) - no extra fragments but needs pre-modified template
 *
 * @param {string} sequence - DNA sequence
 ...
 * @param {string} enzyme - Enzyme name
 * @param {string} organism - Target organism
 * @param {string} codonMode - Codon optimization mode
 * @param {Array} existingOverhangs - Existing overhangs
 * @param {string} selectedStrategy - User's preferred strategy
 */
function generateMutationOptions(sequence, sites, frame, enzyme, organism, codonMode, existingOverhangs, selectedStrategy = ENHANCED_CONFIG.defaultStrategy) {
  const step = {
  ...
  for (const site of sites) {
    ...
    if (junction.success) {
      const mutagenicOption = {
        type: 'mutagenic_junction',
        junction,
        score: junction.combinedScore + (prioritizeMutagenic ? 100 : 0), // Boost if primary strategy
 ...
 */
function generatePreFlightPreview(sequence, sites, siteOptions, frame, enzyme = 'BsaI', selectedStrategy = ENHANCED_CONFIG.defaultStrategy) {
  const step = {
 ...
 */
function findSharedMutation(sequence, site1, site2, frame, enzyme, organism) {
  const codonUsage = organism === 'yeast' ? YEAST_CODON_USAGE : ECOLI_CODON_USAGE;
 ...
 */
function findSingleJunctionForPair(sequence, pair, enzyme, frame, organism) {
 ...
 */
function formatMutationDescription(mutation, codonUsage) {
 ...
 */
function verifyDomestication(originalSeq, domesticatedSeq, frame, enzyme, mutations) {
  const issues = [];
  const checks = [];
