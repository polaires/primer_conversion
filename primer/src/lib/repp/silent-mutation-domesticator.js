 ...
 */
export const ECOLI_CODON_USAGE = {
 ...
 */
export const YEAST_CODON_USAGE = {
...
export const DOMESTICATION_CONFIG = {
 ...
 * @returns {Object} Domestication result with mutated sequence and details
 */
export function domesticateWithSilentMutations(sequence, enzyme = 'BsaI', options = {}) {
  const {
 ...
 *
 * @param {string} sequence - Current DNA sequence
 * @param {Object} site - Site object with position, sequence, orientation
 * @param {number} frame - Reading frame (0, 1, or 2)
 * @param {string} enzyme - Enzyme name
 * @param {Object} codonUsage - Codon usage frequency table
 * @returns {Array} Array of mutation candidates
 */
export function findAllSilentMutationCandidates(sequence, site, frame, enzyme, codonUsage) {
  const candidates = [];
 ...
 *
 * @param {Array} candidates - Array of mutation candidates
 * @param {string} sequence - Current sequence
 * @param {string} enzyme - Current enzyme
 * @param {boolean} checkAllEnzymes - Check for all GG enzyme sites
 * @returns {Array} Sorted array of scored candidates
 */
export function scoreMutationCandidates(candidates, sequence, enzyme, checkAllEnzymes = false) {
  ...
  for (const candidate of candidates) {
    ...
    if (newSitesCreated.createsNewSite) {
      ...
      penalties.push({
        ...
        penalty: 80,
        details: newSitesCreated.newSites,
      });
    }
 ...
 * @param {string} mutatedSeq - Sequence after mutation
 * @param {number} mutationPos - Position of mutation
 * @param {string} currentEnzyme - Current enzyme being used
 * @param {boolean} checkAll - Check all GG enzymes or just current
 * @returns {Object} Analysis of new sites
 */
export function checkForNewSites(originalSeq, mutatedSeq, mutationPos, currentEnzyme, checkAll = false) {
 ...
 * @param {Object} mutation - Mutation object with sequencePosition and newBase
 * @returns {string} Mutated sequence
 */
export function applyMutation(sequence, mutation) {
 ...
 * @returns {string} Mutated sequence
 */
export function applyMutations(sequence, mutations) {
  // Sort by position descending to avoid position shifting issues
  const sorted = [...mutations].sort((a, b) => b.sequencePosition - a.sequencePosition);

  let result = sequence;
  for (const mutation of sorted) {
    result = applyMutation(result, mutation.mutation || mutation);
  }
  return result;
}
 ...
 *
 * @param {string} originalSeq - Original DNA sequence
 * @param {string} domesticatedSeq - Domesticated DNA sequence
 * @param {number} frame - Reading frame
 * @returns {Object} Verification result
 */
export function verifyProteinSequence(originalSeq, domesticatedSeq, frame = 0) {
  const translateSequence = (seq, fr) => {
    const protein = [];
 ...
 *
 * @param {Object} domesticationResult - Result from domesticateWithSilentMutations
 * @param {string} enzyme - Enzyme name
 * @param {Object} options - Validation options
 * @returns {Object} Validation report
 */
export function validateDomestication(domesticationResult, enzyme = 'BsaI', options = {}) {
  const {
    frame = 0,
  ...
  if (siteCheck.hasSites && domesticationResult.failedSites.length === 0) {
    isValid = false;
  }
 ...
 */
function generateSummaryMessage(mutations, failedSites, enzyme) {
  const parts = [];
 ...
 * @returns {Object} Comparison of available strategies
 */
export function compareDomesticationStrategies(sequence, enzyme = 'BsaI', options = {}) {
  ...
  const strategies = {
    silentMutation: {
      ...
      available: silentResult.failedSites.length === 0,
      fragmentIncrease: 0,
      sequenceChange: true,
      proteinChange: false,
