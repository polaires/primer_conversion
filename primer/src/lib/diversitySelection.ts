...
export interface HybridSelectionOptions {
  scoreKey?: string;
  numByScore?: number;
  paretoObjectives?: Array<(candidate: any) => number> | null;
  ranges?: DistanceRanges;
}

export interface SelectedCandidate extends Candidate {
  selectionReason: 'bestOverall' | 'topByScore' | 'paretoOptimal' | 'diverse' | 'included';
}

export type ObjectiveFunction = (candidate: any) => number;
...
// ============================================================================
// Distance and Selection Functions
// ============================================================================
 ...
 */
export function calculateDistance(a: Candidate, b: Candidate, ranges: DistanceRanges): number {
  const {
 ...
 *
 * @param candidates - Array of candidates with score and position info
 * @param numToSelect - Number of candidates to select
 * @param options - Selection options
 * @returns Selected diverse candidates
 */
export function selectDiverseCandidates<T extends Candidate>(
  candidates: T[],
  numToSelect: number,
  ...
  const sorted = [...candidates].sort((a, b) => (b as any)[scoreKey] - (a as any)[scoreKey]);

  // Filter by minimum score FIRST (before any early returns)
  const viable = sorted.filter(c => (c as any)[scoreKey] >= minScore);
  ...
  while (selected.length < numToSelect && remaining.length > 0) {
    ...
    for (let i = 0; i < remaining.length; i++) {
      ...
      const maxScore = (viable[0] as any)[scoreKey];
      const minViableScore = (viable[viable.length - 1] as any)[scoreKey];
      const scoreRange = maxScore - minViableScore || 1;
      const rawQuality = ((candidate as any)[scoreKey] - minViableScore) / scoreRange;
 ...
 * @param objectives - Array of objective functions (higher = better)
 * @returns Pareto-optimal candidates
 */
export function selectParetoOptimal<T>(
  candidates: T[],
 ...
 *
 * @param candidates - Array of candidates
 * @param numToSelect - Total number to select
 * @param options - Selection options
 * @returns Selected candidates with labels
 */
export function selectHybrid<T extends Candidate>(
  candidates: T[],
  numToSelect: number,
  options: HybridSelectionOptions = {}
): (T & { selectionReason: string })[] {
  const {
    scoreKey = 'score',
  ...
  const selected: (T & { selectionReason: string })[] = [];
  ...
  if (numRemaining > 0) {
    const remaining = sorted.filter(c =>
      !selected.some(s => s === c || ((s as any).id && (s as any).id === (c as any).id))
    );
...
// ============================================================================
// Strength and Badge Identification
// ============================================================================
 ...
 * @param allCandidates - All candidates for comparison
 * @param index - Index of this candidate in the list
 * @returns Array of strength identifiers (usually 0-1 items)
 */
export function identifyStrengths(candidate: any, allCandidates: any[], index: number = -1): string[] {
 ...
 *
 * @param pair - Primer pair with forward/reverse primers
 * @returns Array of badge objects with id, svgPath, label, and tooltip
 */
export function identifyBadges(pair: any): BadgeInfo[] {
...
// ============================================================================
// Label and Explanation Generation
// ============================================================================

/**
 * Label SVG icon paths for professional rendering
 */
export const LABEL_SVG_PATHS: Record<string, string> = {
 ...
 *
 * @param strengths - Array of strength identifiers
 * @param tier - Quality tier (excellent, good, etc.) - not shown in label
 * @returns Label object with {key, text, svgPath}, or null if no unique strength
 */
export function generateLabel(strengths: string[], tier: string = ''): LabelInfo | null {
 ...
 * @param strengths - Its identified strengths
 * @param reference - Reference candidate for comparison
 * @returns Explanation text or null if nothing useful to add
 */
export function generateExplanation(candidate: any, strengths: string[], reference: any = null): string | null {
