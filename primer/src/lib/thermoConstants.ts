...
/**
 * Thermodynamic parameter pair: [ΔH (kcal/mol), ΔS (cal/mol·K)]
 */
export type ThermoParamArray = [number, number];

/**
 * Thermodynamic parameter object: { dH: kcal/mol, dS: cal/(mol·K) }
 */
export interface ThermoParamObject {
  dH: number;
  dS: number;
}

/**
 * Nearest-neighbor parameter map (array format)
 */
export type NNParamsArray = Record<string, ThermoParamArray>;

/**
 * Nearest-neighbor parameter map (object format)
 */
export type NNParamsObject = Record<string, ThermoParamObject>;

/**
 * Owczarzy Mg²⁺ correction coefficients
 */
export interface OwczarzyCoefficients {
  a: number;
  b: number;
  c: number;
  d: number;
 ...
 */
export const NN_PARAMS_ARRAY: NNParamsArray = {
  'AA': [-7.6, -21.3],
  'TT': [-7.6, -21.3],
  'AT': [-7.2, -20.4],
  'TA': [-7.2, -21.3],
 ...
 */
export const NN_PARAMS_OBJECT: NNParamsObject = {
  'AA': { dH: -7.6, dS: -21.3 },
  'TT': { dH: -7.6, dS: -21.3 },
  'AT': { dH: -7.2, dS: -20.4 },
  'TA': { dH: -7.2, dS: -21.3 },
 ...
 */
export const OWCZARZY_COEFFICIENTS: OwczarzyCoefficients = {
  a: 3.92e-5,
  b: -9.11e-6,
  c: 6.26e-5,
  d: 1.42e-5,
 ...
 */
export const NN_MISMATCH: NNParamsObject = {
 ...
 */
export const DANGLING_END_CORRECTIONS: Record<string, ThermoParamObject> = {
