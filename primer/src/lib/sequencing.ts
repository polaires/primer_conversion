...
export interface SequencingDefaults {
  minPrimerLength: number;
  maxPrimerLength: number;
  optimalPrimerLength: number;
  minTm: number;
  maxTm: number;
  optimalTm: number;
  minGC: number;
  ...
  minGCinLast5: number;
  maxGCinLast5: number;
  maxConsecutiveGCat3prime: number;
  maxPolyN: number;
  maxDinucleotideRepeat: number;
  maxHairpinDG: number;
  maxInternalHairpinDG: number;
  maxSelfDimerDG: number;
  maxInternalDimerDG: number;
  maxTmImbalance: number;
  checkOffTargets: boolean;
  maxOffTargets: number;
  readStartOffset: number;
  optimalReadLength: number;
  minReadLength: number;
  maxReadLength: number;
  primerSpacing: number;
  spacingTolerance: number;
  minOverlap: number;
  bidirectionalCoverage: boolean;
  ensureEdgeCoverage: boolean;
  circular: boolean;
...
export interface GCClampAnalysis {
  gcInLast5: number;
  consecutiveGCat3: number;
  last5: string;
  hasProperClamp: boolean;
  lastBase: string;
  hasGCatEnd: boolean;
}

export interface TmBalance {
  tm5prime: number;
  tm3prime: number;
  difference: number;
  isBalanced: boolean;
...
export interface PrimerQuality {
  ...
  tmBalance: TmBalance;
  homopolymers: { base: string; length: number; position: number } | null;
  dinucRepeats: { pattern: string; sequence: string; units: number; position: number } | null;
  offTargetCount: number;
  gQuadruplex?: any;
  analysis3Prime?: any;
  hasGCClamp: boolean;
  hasStrongGCClamp: boolean;
  quality: string;
  penalty: number;
  issues: string[];
  warnings: string[];
...
export interface Primer extends PrimerQuality {
  position: number;
...
export interface TmCompatibility {
  ...
  mean: number;
  stdDev: number;
  outliers: Array<{ name: string; tm: number; deviation: number }>;
...
export const SEQUENCING_DEFAULTS: SequencingDefaults = {
 ...
 */
export function calculateTmNN(seq: string, primerConc: number = 0.25e-6): number {
 ...
 */
function isSymmetric(seq: string): boolean {
 ...
 */
function calculateDeltaG(seq: string, tempC: number = 37): number {
  const sequence = seq.toUpperCase();
...
function prepareCircularWorkingSequence(
  seq: string,
  circular: boolean
): { workingSeq: string; originalLength: number; isCircular: boolean } {
  if (!circular) {
    return {
      workingSeq: seq,
 ...
 */
function adjustCircularCoordinates(primer: Primer, seqLength: number): Primer {
  const result = { ...primer };
 ...
 */
function circularPosition(position: number, seqLength: number, circular: boolean): number {
 ...
 */
function detectHomopolymers(seq: string): Array<{ base: string; length: number; position: number }> {
 ...
 */
function detectDinucleotideRepeats(seq: string): Array<{ pattern: string; sequence: string; units: number; position: number }> {
  const sequence = seq.toUpperCase();
  const repeats: Array<{ pattern: string; sequence: string; units: number; position: number }> = [];
 ...
 */
function analyzeGCClamp(seq: string): GCClampAnalysis {
  const sequence = seq.toUpperCase();
 ...
 */
function calculateTmBalance(seq: string): TmBalance {
  const sequence = seq.toUpperCase();
...
export function scorePrimerQuality(
  sequence: string,
  options: Partial<SequencingDefaults> = {},
  template: string | null = null
): PrimerQuality {
  const opts: SequencingDefaults = { ...SEQUENCING_DEFAULTS, ...options };
 ...
 */
function analyzeTmCompatibility(primers: Primer[], opts: SequencingDefaults): TmCompatibility {
  if (primers.length === 0) {
    return { compatible: true, range: 0, minTm: 0, maxTm: 0, mean: 0, stdDev: 0, outliers: [], recommendation: '' };
  }
 ...
 */
export function designPrimerAtPosition(
  template: string,
  position: number,
...
function calculateCoverage(primers: Primer[], templateLength: number, opts: SequencingDefaults): Coverage {
...
function findGaps(coverageArray: number[]): Array<{ start: number; end: number; length: number }> {
 ...
 */
export function detectHairpins(seq: string): {
  hairpins: Array<{ dG: number; is3prime: boolean }>;
  worstHairpin: { dG: number } | null;
  worst3primeHairpin: { dG: number } | null;
  hasProblem: boolean;
  _deprecated: boolean;
  _useInstead: string;
} {
 ...
 */
export function detectSelfDimer(seq: string): {
  dimers: Array<{ dG: number; is3prime: boolean }>;
  worstDimer: { dG: number } | null;
  worst3primeDimer: { dG: number } | null;
  hasProblem: boolean;
  _deprecated: boolean;
  _useInstead: string;
} {
