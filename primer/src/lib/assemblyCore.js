 ...
 * - primers.js (primer design core)
 * - tmQ5.js (Q5 Tm calculation)
 * - equilibrium.js (thermodynamic analysis)
 ...
 *
 * @param {string} fragment1End - 3' end of fragment 1 (last ~50bp)
 * @param {string} fragment2Start - 5' end of fragment 2 (first ~50bp)
 * @param {Object} config - Assembly configuration
 * @returns {Object} Optimal overlap specification
 */
export function findOptimalOverlap(fragment1End, fragment2Start, config = {}) {
  ...
  for (let len = minLen; len <= maxLen; len++) {
    ...
    candidates.push({
      ...
      warnings: [
        ...(hasPolyT ? ['Contains TTTT - may cause poor extension'] : []),
        ...(hasPolyG ? ['Contains GGGG - G-quadruplex risk'] : []),
        ...(tm < method.overlapTm.min ? [`Tm ${tm.toFixed(1)}°C below minimum ${method.overlapTm.min}°C`] : []),
        ...(tm > method.overlapTm.max ? [`Tm ${tm.toFixed(1)}°C above maximum ${method.overlapTm.max}°C`] : []),
      ],
    });
  }
 ...
 *
 * @param {number} tm - Melting temperature
 * @param {Object} method - Assembly method config
 * @returns {number} Score 0-1
 */
function scoreOverlapTmRange(tm, method) {
 ...
 *
 * @param {number} length - Overlap length in bp
 * @param {Object} method - Assembly method config
 * @returns {number} Score 0-1
 */
function scoreOverlapLengthRange(length, method) {
 ...
 *
 * @param {string} seq - Overlap sequence
 * @returns {Object} { score: 0-1, patterns: Object }
 */
function scorePatternAvoidance(seq) {
  let score = 1.0;
  const patterns = {
    polyT: /TTTT/.test(seq),
    polyA: /AAAA/.test(seq),
 ...
 *
 * @param {string} seq - Sequence to analyze
 * @returns {number} Score 0-1 (1.0 = no self-complementarity)
 */
function scoreSelfComplementarity(seq) {
 ...
 *
 * @param {string} frag1End - 3' end of fragment 1
 * @param {string} frag2Start - 5' start of fragment 2
 * @param {number} overlapLen - Length of overlap
 * @returns {number} Score 0-1 (1.0 = no cross-junction hairpin)
 */
function scoreCrossJunctionHairpin(frag1End, frag2Start, overlapLen) {
 ...
 * @param {string} fragment2Start - 5' end of fragment 2 (first ~50bp)
 * @param {Object} config - Assembly configuration
 * @returns {Object} Optimal overlap with quality metrics
 */
export function findOptimalOverlapRangeBased(fragment1End, fragment2Start, config = {}) {
  const cfg = { ...DEFAULT_ASSEMBLY_CONFIG, ...config };
  const method = ASSEMBLY_METHODS[cfg.method] || ASSEMBLY_METHODS.NEBUILDER_HIFI;
 ...
 */
function generateDiverseOverlapAlternatives(validCandidates, best) {
  ...
  if (shortestValid && !seen.has(shortestValid.sequence)) {
    alternatives.push({ ...shortestValid, selectionReason: 'Shortest overlap (lower primer cost)' });
  ...
  const bestHairpin = validCandidates
    ...
    .sort((a, b) => b.scores.hairpin - a.scores.hairpin)[0];
  if (bestHairpin && bestHairpin.scores.hairpin > (best?.scores?.hairpin || 0)) {
    alternatives.push({ ...bestHairpin, selectionReason: 'Best secondary structure avoidance' });
  ...
  const best3Prime = validCandidates
    .filter(c => !seen.has(c.sequence))
    .sort((a, b) => (b.scores.gcClamp + b.scores.terminal3DG) - (a.scores.gcClamp + a.scores.terminal3DG))[0];
  if (best3Prime) {
    alternatives.push({ ...best3Prime, selectionReason: 'Best 3\' end stability' });
  ...
  if (mostRobust) {
    alternatives.push({ ...mostRobust, selectionReason: 'Most robust Tm (middle of range)' });
 ...
 *
 * @param {Array} fragments - Array of {id, seq} fragment objects
 * @param {Object} config - Assembly configuration
 * @returns {Object} Optimized assembly plan
 */
export function optimizeAssemblyOverlaps(fragments, config = {}) {
  const cfg = { ...DEFAULT_ASSEMBLY_CONFIG, ...config };
  ...
  const getScore = (j) => j.overlap.compositeScore ?? j.overlap.score ?? 0;
 ...
 *
 * @param {string} fragmentSeq - Fragment sequence to amplify
 * @param {Object} context - Assembly context
 * @param {string} context.leftOverlap - Sequence to add to 5' (homology tail)
 * @param {string} context.rightOverlap - Sequence to add to 3' (homology tail)
 * @param {Object} options - Primer design options
 * @returns {Object} Designed primers
 */
export function designAssemblyPrimers(fragmentSeq, context, options = {}) {
  const {
    leftOverlap = '',
    rightOverlap = '',
    method = 'NEBUILDER_HIFI',
  ...
  const cfg = { ...DEFAULT_ASSEMBLY_CONFIG, ...options };
 ...
 *
 * @param {string} fragmentSeq - Fragment sequence to amplify
 * @param {Object} context - Assembly context with overlap info
 * @param {Object} options - Design options
 * @returns {Object} Optimized assembly primers
 */
export function designAssemblyPrimersOptimized(fragmentSeq, context, options = {}) {
  const {
    leftOverlap = '',
    rightOverlap = '',
    method = 'NEBUILDER_HIFI',
 ...
 */
function generateAnnealingCandidates(fragmentSeq, isFwd, ranges, overlapTail) {
  const candidates = [];
 ...
 */
function findBestPrimerPair(fwdCandidates, revCandidates, leftOverlap, rightOverlap, cfg) {
 ...
 * @param {Array} fragments - Array of {id, seq} fragment objects
 * @param {Object} options - Design options
 * @returns {Object} Complete assembly design with all primers
 */
export function designAssembly(fragments, options = {}) {
  const cfg = { ...DEFAULT_ASSEMBLY_CONFIG, ...options };
  const method = ASSEMBLY_METHODS[cfg.method] || ASSEMBLY_METHODS.NEBUILDER_HIFI;
 ...
 */
function generateProtocol(method, fragmentDesigns) {
 ...
 */
export function exportPrimers(assemblyDesign, format = 'tsv') {
  const rows = [];
 ...
 *
 * @param {Array} fragments - Array of {id, seq} fragment objects
 * @param {Array} junctions - Junction data from assembly design
 * @param {Object} options - Simulation options
 * @returns {Object} Simulated assembly result
 */
export function simulateAssembly(fragments, junctions, options = {}) {
  ...
  for (let i = 0; i < fragments.length; i++) {
    ...
    if (i === 0) {
      ...
      const overlapLen = junction?.overlap?.length || junction?.optimal?.length || 20;
  ...
  if (circular) {
    ...
    const overlapLen = lastJunction?.overlap?.length || lastJunction?.optimal?.length || 20;
 ...
 */
function detectHomologyConflicts(fragments, junctions) {
  const conflicts = [];
 ...
 */
function findSharedHomology(seq1, seq2, minLength) {
 ...
 */
function generateVisualizationData(fragments, junctions, options = {}) {
  ...
  const junctionPositions = junctions.map((j, i) => {
    const frag = fragmentPositions[i];
    return {
      index: i,
      from: j.from || fragments[i]?.id,
      to: j.to || fragments[(i + 1) % fragments.length]?.id,
      position: frag.endPosition - (j.overlap?.length || j.optimal?.length || 20),
      overlapSequence: j.overlap?.sequence || j.optimal?.sequence,
      quality: j.overlap?.quality || j.optimal?.quality || 'unknown',
      score: j.overlap?.score || j.optimal?.compositeScore,
    };
  });
 ...
 * @returns {Object} Validation result with junction analysis
 */
export function validateJunctions(fragments, options = {}) {
 ...
 *
 * @param {Object} simulation - Simulation result from simulateAssembly()
 * @param {Object} options - Export options
 * @returns {string} GenBank formatted string
 */
export function exportToGenBank(simulation, options = {}) {
  const {
    name = 'Assembly',
 ...
 */
export function exportToFasta(simulation, options = {}) {
  const {
    name = 'Assembly',
    description = '',
    lineWidth = 70,
 ...
 * @param {Object} simulation - Simulation result
 * @param {Object} options - Export options
 * @returns {Object} Export data with GenBank as primary format
 */
export function exportToSnapGene(simulation, options = {}) {
