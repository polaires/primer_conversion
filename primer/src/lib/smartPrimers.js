 ...
 */
export const SMART_DESIGN_CONSTRAINTS = {
  // Length bounds
  minLength: 15,
  maxLength: 32,
 ...
 */
export function analyze3PrimeEnd(seq) {
  seq = seq.toUpperCase();
 ...
 * @param {number} params.energy - Hairpin ΔG in kcal/mol
 * @param {Array} params.basePairs - Array of [i, j] base pair positions
 * @param {number} params.seqLength - Total sequence length
 * @param {Object} params.structure - Optional parsed structure with has3PrimeInStem, has3PrimeInLoop
 * @returns {Object} Severity classification with level, label, message, tooltip, color
 */
export function classify3PrimeStructureSeverity({ energy, basePairs = [], seqLength, structure = null }) {
 ...
 *
 * @param {Object} params - Same as classify3PrimeStructureSeverity
 * @returns {Object|null} Badge object or null if no badge needed
 */
export function get3PrimeStructureBadge({ energy, basePairs = [], seqLength, structure = null }) {
 ...
 * @param {number} startPos - Start position of primer on template
 * @param {number} currentLength - Current primer length
 * @param {boolean} isFwd - Is forward primer
 * @param {Object} options - Generation options
 * @returns {Array} Array of variant sequences with metadata
 */
export function generateLengthVariants(fullSeq, startPos, currentLength, isFwd, options = {}) {
  const {
 ...
 *
 * @param {Object} analysis - 3' end analysis
 * @param {number} lengthDelta - Change in length from original
 * @returns {number} Priority score (higher is better)
 */
function calculateVariantPriority(analysis, lengthDelta) {
  let priority = 0;
 ...
 * @returns {Object} Optimized primer with rationale
 */
export function optimizePrimer(fullSeq, startPos, currentLength, isFwd, options = {}) {
  const {
    targetGcClamp = true,
    target3PrimeDG = true,
    maxLengthChange = 3,
 ...
 *
 * @param {string} seq - Primer sequence
 * @param {string} template - Full template sequence (for off-target)
 * @param {boolean} isFwd - Is forward primer
 * @param {number} temperature - Annealing temperature
 * @returns {Object} Comprehensive scoring
 */
export function scorePrimerVariant(seq, template = '', isFwd = true, temperature = 55) {
 ...
 *
 * @param {Object} analysis - Result from analyze3PrimeEnd()
 * @returns {number} Score from 0 to 1
 */
export function score3PrimeComposition(analysis) {
  let score = 1.0;
 ...
 *
 * @param {Object} primerScore - Result from scorePrimerVariant()
 * @returns {Array} Array of actionable suggestions
 */
export function generateDesignSuggestions(primerScore) {
  const suggestions = [];
 ...
 *
 * @param {string} seq - Target sequence to amplify
 * @param {Object} initialFwd - Initial forward primer {seq, startPos, length}
 * @param {Object} initialRev - Initial reverse primer {seq, startPos, length}
 * @param {string} fullTemplate - Full template sequence
 * @param {Object} options - Optimization options
 * @returns {Object} Optimized primer pair with design rationale
 */
export function optimizePrimerPair(seq, initialFwd, initialRev, fullTemplate, options = {}) {
  const {
 ...
 */
export function quickAssess(seq) {
  const analysis = analyze3PrimeEnd(seq);
  const gcContent = (seq.match(/[GC]/gi) || []).length / seq.length;
  const primerTm = tm(seq, '', true);
 ...
 * @returns {string} Suggested optimization approach
 */
function suggestOptimizationStrategy(issues) {
  const strategies = [];
