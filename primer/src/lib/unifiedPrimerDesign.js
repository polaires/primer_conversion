 ...
 */
function extractCircularRegion(seq, start, end, circular) {
  if (start <= end) {
 ...
 * @param {number} start - Start position (0-based)
 * @param {number} end - End position (0-based, exclusive)
 * @param {number} seqLength - Total sequence length
 * @param {boolean} circular - Whether sequence is circular
 * @returns {number} Length of the region
 */
function getCircularRegionLength(start, end, seqLength, circular) {
  if (start <= end) {
    return end - start;
  }
 ...
 * @param {number} end - End position (0-based, exclusive)
 * @param {boolean} circular - Whether sequence is circular
 * @returns {Object} { workingSeq, workingStart, workingEnd, isWrapped }
 */
function prepareCircularWorkingSequence(seq, start, end, circular) {
  if (start <= end) {
 ...
 *
 * @param {Object} primer - Primer object with start/end coordinates
 ...
 * @param {number} spec.start - Start position (0-based)
 * @param {number} spec.end - End position (0-based, exclusive)
 * @param {string|null} spec.replacement - What to put at the target region:
 *   - null/undefined: Amplify (extract the region)
 *   - "": Delete the region
 *   - "ACGT": Substitute with this sequence
 * @param {Object} [spec.aaHelper] - Optional amino acid helper
 * @param {string} spec.aaHelper.newAA - Target amino acid (single letter)
 * @param {string} [spec.aaHelper.organism='ecoli'] - Organism for codon optimization
 * @param {Object} [options] - Design options (Tm, GC, etc.)
 * @param {boolean} [options.circular=true] - Whether template is circular (plasmid)
 * @returns {Object} Design result with primers and analysis
 */
export function designUnified(template, spec, options = {}) {
 ...
 */
function determineOperationType(spec) {
  const { start, end, replacement, aaHelper } = spec;
 ...
 */
function designAmplification(template, start, end, options, isCircular = true) {
  ...
  if (fwd.alternatives && fwd.alternatives.length > 0) {
    alternativePrimers = fwd.alternatives
      ...
      .filter(alt => Math.abs(alt.forward.tm - alt.reverse.tm) <= MAX_TM_DIFF)
      // Add comparison info
      .map(alt => ({
        ...alt,
  ...
  if (fwd.alternativeOptimized && alternativePrimers.length < 10) {
    ...
    const isDuplicate = alternativePrimers.some(alt =>
 ...
 */
function designDeletion(template, start, end, options, isCircular = true) {
  const seqLen = template.length;
  const isWrapped = start > end;
 ...
 */
function designSubstitution(template, start, end, replacement, options, isCircular = true) {
  const seqLen = template.length;
 ...
 */
function designAAMutation(template, start, aaHelper, options) {
 ...
 * @param {Array<Object>} specs - Array of design specifications
 * @param {Object} options - Shared design options
 * @returns {Array<Object>} Array of design results
 */
export function designBatch(template, specs, options = {}) {
  const results = [];
 ...
 *
 * @param {string} notation - Mutation notation
 * @param {Object} context - Context information
 * @param {number} [context.orfStart=1] - ORF start position
 * @returns {Object} Unified specification
 */
export function parseNotationToSpec(notation, context = {}) {
 ...
 * # This is a comment
 */
export function parseBatchText(text, defaultOptions = {}) {
