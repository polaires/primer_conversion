...
/**
 * Thermodynamic thresholds for structure analysis
 */
export interface AnalysisThresholds {
  hairpin: number;
  hairpin3Prime: number;
  homodimer: number;
  homodimer3Prime: number;
  heterodimer: number;
  heterodimer3Prime: number;
  hairpinCritical?: number;
  homodimerCritical?: number;
  heterodimerCritical?: number;
}
...
export interface AnalysisPreset {
  ...
  optimalTmRange: [number, number];
  optimalGcRange: [number, number];
  optimalLengthRange: [number, number];
  tmMethod: string;
  weights: ScoringWeights;
  // Scoring function options (for backward compatibility)
  tmOptions: ScoringOptions;
  gcOptions: ScoringOptions;
  lengthOptions: ScoringOptions;
...
/**
 * Weight multipliers for application-specific tuning
 */
 ...
 * Preset mode type
 */
export type PresetMode = 'amplification' | 'mutagenesis' | 'sequencing' | 'assembly' | 'goldengate';

/**
 * Application preset ID type
 */
export type ApplicationPresetId = 'default' | 'qpcr' | 'colonyPcr' | 'longRange' | 'gcRich' | 'atRich' | 'multiplex' | 'highFidelity';
 ...
 */
export const ANALYSIS_PRESETS: Record<PresetMode, AnalysisPreset> = {
  amplification: {
 ...
 */
export function getPreset(mode: PresetMode | string): AnalysisPreset {
  return ANALYSIS_PRESETS[mode as PresetMode] || ANALYSIS_PRESETS.amplification;
}
 ...
 */
export const APPLICATION_PRESETS: Record<ApplicationPresetId, ApplicationPreset> = {
  default: {
    id: 'default',
    name: 'Standard PCR',
 ...
 * @returns Array of preset objects with id, name, description
 */
export function getAvailablePresets(): PresetListItem[] {
 ...
 * @returns Recommended preset ID
 */
export function autoDetectPreset(params: AutoDetectParams = {}): ApplicationPresetId {
  const { sequence = '', ampliconLength = 0, applicationHint = '' } = params;
